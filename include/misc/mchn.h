...
struct mchn_ops {
	 ...
	 */
	int (*pop_link)(int, struct mch_buf *, struct mch_buf *, int);
	/* ap don't need to implementation */
	int (*push_link)(int, struct mch_buf **, struct mch_buf **, int *);
...
/* push link list */
int mchn_push_link(int channel, struct mch_buf *head,
		   struct mch_buf *tail, int num);
...
int mbuf_link_alloc(int chn, struct mch_buf **head, struct mch_buf **tail,
		     int *num);
int mbuf_link_free(int chn, struct mch_buf *head,
		   struct mch_buf *tail, int num);
