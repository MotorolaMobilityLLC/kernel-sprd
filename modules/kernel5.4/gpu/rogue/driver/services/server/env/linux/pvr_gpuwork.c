...
static IMG_UINT32 _GetUID(IMG_PID pid)
{
	...
	psPid = pvr_find_get_pid((pid_t)pid);
	if (!psPid)
	{
		...
		PVR_GPU_WORK_EVENT_TYPE eEventType)
{
	...
	eError =_InsertGpuWorkPeriodEvent(psGpuWorkPeriodEventStats,
			u32GpuId, ui32IntJobRef, ui64HWTimestampInOSTime, eEventType);
...
_EmitCallback(uintptr_t k, uintptr_t v, void *argv)
{
	...
	while (psEmitListHead)
	{
		...
		const IMG_UINT64 maxActiveDurationInNs = 1000000000U;
		const IMG_UINT64 adjustBiasInNs = 1U;
		...
		if (ui64StartTimeInNs < psGpuWorkPeriodEventStats->ui64LastActiveTimeInNs)
		{
			ui64StartTimeInNs = psGpuWorkPeriodEventStats->ui64LastActiveTimeInNs + adjustBiasInNs;
		}

		if (ui64EndTimeInNs < ui64StartTimeInNs)
		{
			ui64EndTimeInNs = ui64StartTimeInNs + adjustBiasInNs;
		}

		if (ui64EndTimeInNs - ui64StartTimeInNs > maxActiveDurationInNs)
		{
			ui64EndTimeInNs = ui64StartTimeInNs + maxActiveDurationInNs - adjustBiasInNs;
		}

		trace_gpu_work_period(psEmitListHead->ui32GpuId,
				(IMG_UINT64)ui32Uid, ui64StartTimeInNs,
				ui64EndTimeInNs + adjustBiasInNs, ui64EndTimeInNs - ui64StartTimeInNs);

		psGpuWorkPeriodEventStats->ui64LastActiveTimeInNs = ui64EndTimeInNs + adjustBiasInNs;
...
PVRSRV_ERROR GpuTraceWorkPeriodInitialize(void)
{
	...
	if (psGpuWorkPeriodEventData->bInitialized == IMG_FALSE)
	{
		...
		PVR_LOG_GOTO_IF_ERROR(eError, "OSEventObjectCreate", err_out);
