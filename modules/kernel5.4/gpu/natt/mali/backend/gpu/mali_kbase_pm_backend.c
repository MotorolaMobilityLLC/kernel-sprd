...
int kbase_pm_runtime_init(struct kbase_device *kbdev)
{
	...
	if (callbacks) {
		...
		kbdev->pm.backend.callback_power_on_first_part =
					callbacks->power_on_first_part_callback;
		kbdev->pm.backend.callback_power_on_clear_flag =
					callbacks->power_on_clear_flag_callback;
...
void kbase_pm_register_access_enable(struct kbase_device *kbdev)
{
	...
	if (callbacks->power_on_first_part_callback)
		callbacks->power_on_first_part_callback(kbdev);

	if (callbacks)
		callbacks->power_on_callback(kbdev);

	if (callbacks->power_on_clear_flag_callback)
		callbacks->power_on_clear_flag_callback(kbdev);
...
static void kbase_pm_gpu_poweroff_wait_wq(struct work_struct *data)
{
	...
	if (is_gpu_powered_down(kbdev)) {
		if (backend->callback_power_off_second_part)
			backend->callback_power_off_second_part(kbdev);
		else
			printk("%s:backend->callback_power_off_second_part is NULL", __func__);
	} else
		printk(KERN_ERR "%s don't need to disable vddgpu, because gpu didn't powered_down.\n", __func__);
...
int kbase_hwaccess_pm_suspend(struct kbase_device *kbdev)
{
	...
	if (kbdev->pm.backend.callback_power_off_second_part) {
		kbdev->pm.backend.callback_power_off_second_part(kbdev);
		printk("mali kbase_device_pm_suspend");
	}
