...
#define TIMEOUT 300000000
#define PROCESS_TIMEOUT(a, b) ((b != 0) && (a - b > TIMEOUT))
...
#undef compiletime_assert_defined
		...
		do {
			...
			if (datom && (datom->job_process_timestamp.job_submit_time != 0))
				kbase_atom_dump_execption(datom, true);
		} while (atom_number_--);
...
void kbase_atom_dump_execption(struct kbase_jd_atom *katom, bool reset)
{
	...
	if (katom->run_status & KRun_EventPost) {
		if (PROCESS_TIMEOUT(katom->job_process_timestamp.post_event_time, katom->job_process_timestamp.job_submit_time)) {
			time_show = true;
			printk("KM_POST_EVENT Timeout state: atom:%d time processing = %ldns", katom->atom_number,
					katom->job_process_timestamp.post_event_time - katom->job_process_timestamp.job_submit_time);
		}
		...
		if ((katom->core_req & BASE_JD_REQ_EVENT_ONLY_ON_FAILURE) ||
			...
			//printk("Don't report the event until other event have completed");
		...
		if (katom->early_brk) {
			clear = true;
			if (!reset)
				printk("Terminating state: the atom:%d returned early", katom->atom_number);
		} else {
			if (PROCESS_TIMEOUT(current_time, katom->job_process_timestamp.job_submit_time)) {
				time_show = true;
				printk("KM_POST_EVENT Pending state: atom:%d time pending = %ldns",
						katom->atom_number, current_time - katom->job_process_timestamp.job_submit_time);
			}
		}
	}
	...
	if (wakeup) {
		...
		if (katom->run_status & kRun_EventProcess) {
			clear = true;
			if (PROCESS_TIMEOUT(katom->job_process_timestamp.process_event_time, katom->job_process_timestamp.wake_up_time)) {
				time_show = true;
				printk("KM_PROCESS_EVENT timeout state, atom:%d coalesce:%s time_Process = %ldns",
						...
						katom->job_process_timestamp.process_event_time - katom->job_process_timestamp.wake_up_time);
			}
		} else {
			if (PROCESS_TIMEOUT(current_time, katom->job_process_timestamp.wake_up_time)) {
				time_show = true;
				printk("KM_PROCESS_EVENT pending state, atom:%d coalesce:%s time_pending = %ldns",
						katom->atom_number,  katom->core_req & BASE_JD_REQ_EVENT_COALESCE ? "true" : "false",
						current_time - katom->job_process_timestamp.wake_up_time);
			}
		}
	}

	// In some cases, the process event has already been executed
	if (katom->status & kRun_EventProcess)
		clear = true;
	...
	if (katom->core_req & BASE_JD_REQ_SOFT_JOB)
		job_type |= 0x1;
	else if ((katom->core_req & BASE_JD_REQ_HARD_JOB) || (katom->core_req == BASE_JD_REQ_DEP))
		job_type |= 0x2;

	printk("Dump job infos: {pid:%d tgid:%d power:%d} katom{0x%p atom_number{%d} job_type{sw:%d hw:%d}}, core_req=0x%x, status=0x%x, event_code=0x%x, atom_flags=0x%x, run_status=0x%x, gpu_rb_state=0x%x, will_fail_event_code=0x%x",
			kctx->pid, kctx->tgid, kbdev->poweroff_pending ? 0 : 1, katom, katom->atom_number, job_type & 0x1 ? 1 : 0, job_type & 0x2 ? 1 : 0,
			katom->core_req, katom->status, katom->event_code, katom->atom_flags, katom->run_status, katom->gpu_rb_state, katom->will_fail_event_code);
	...
	case BASE_JD_REQ_SOFT_EVENT_WAIT:
		printk("software job infos: work{ jd_req_soft_event_set:%d ioctl:%d soft_event(q:%ldns, w:%ldns) }",
			katom->core_req & BASE_JD_REQ_SOFT_EVENT_SET ? 1 : 0, kctx->ioctl & KBASE_IOCTL_SOFT_EVENT_UPDATE ? 1 : 0,
			katom->soft_event_complete_work.queue_work_time, katom->soft_event_complete_work.done_work_time);
		break;
	case BASE_JD_REQ_SOFT_FENCE_TRIGGER:
		fence = dma_fence_get(katom->dma_fence.fence);
		if (fence) {
			err = dma_fence_get_status(fence);
			printk("software job infos: fence trigger{ fence_status(0x%p %s %s:%s:%llx:%llx err:%d, flags:%lu, timestamp:%llx) signal_time:%ldns }",
				fence, dma_fence_is_signaled(fence) ? "signaled" : "active", fence->ops->get_driver_name(fence), fence->ops->get_timeline_name(fence),
				fence->context, fence->seqno, err, fence->flags, fence->timestamp,
				katom->job_process_timestamp.fence_trigger_signal_time);
		} else {
			printk("software job infos: no fence in fence trigger or fence has signaled, run:%d signal_time:%ldns",
					katom->run_status & kRun_FenceOutTrigger, katom->job_process_timestamp.fence_trigger_signal_time);
		}
		break;
	case BASE_JD_REQ_SOFT_FENCE_WAIT:
		...
		if (fence) {
			err = dma_fence_get_status(fence);
			printk("software job infos: work{ fence_status(0x%p %s %s:%s:%llx:%llx err:%d, flags:%lu, timestamp:%llx) soft_fence(q:%ldns, w:%ldns) }",
				fence, dma_fence_is_signaled(fence) ? "signaled" : "active", fence->ops->get_driver_name(fence), fence->ops->get_timeline_name(fence),
				fence->context, fence->seqno, err, fence->flags, fence->timestamp,
				katom->sync_wait_fence_work.queue_work_time, katom->sync_wait_fence_work.done_work_time);
		} else {
			printk("software job infos: no fence in fence wait or fence has signaled, run:%d soft_fence:%ldns",
	...
	// hardware job
	if (job_type & 0x2)  {
		printk("hardware job infos: work{ hw_job(js:%ldns, start:%ldns, end:%ldns, code:0x%x), jd_done_work(q:%ldns, w:%ldns) }",
			katom->job_process_timestamp.hw_job_start_time, katom->start_timestamp, katom->job_process_timestamp.hw_job_end_time,
			katom->completion_code, katom->jd_done_work.queue_work_time, katom->jd_done_work.done_work_time);
	}

	if (katom->core_req & BASE_JD_REQ_EXTERNAL_RESOURCES)
		printk("external resources: dma fence may exist, work{ dma_fence_work(q:%ldns, w:%ldns) }",
			katom->dma_fence_work.queue_work_time, katom->dma_fence_work.done_work_time);

	printk("Process timestamp: job_submit(%ldns), post_event(%ldns), wakeup_time(%ldns), js_complete(%ldns), jd_nolock(%ldns), process_event(%ldns), current_time(%ldns)",
		katom->job_process_timestamp.job_submit_time, katom->job_process_timestamp.post_event_time, katom->job_process_timestamp.wake_up_time,
		katom->job_process_timestamp.js_complete_time, katom->job_process_timestamp.job_nolock_time, katom->job_process_timestamp.process_event_time, current_time);

	for (i = 0; i < 2; i++) {
		struct kbase_jd_atom *atom = katom->dep[i].atom;
		if (atom) {
			printk("Dependency atom[%d], dumping......", i);
			kbase_atom_dump_execption(atom, false);
			//printk("Dependency atom[%d]: dep_type:%d, katom{0x%p atom_number{%d} }, core_req=0x%x, status=0x%x, event_code=0x%x, run_status=0x%x",
					//i, katom->dep[i].dep_type, atom, atom->atom_number, atom->core_req, atom->status, atom->event_code, atom->run_status);
		}
	}
