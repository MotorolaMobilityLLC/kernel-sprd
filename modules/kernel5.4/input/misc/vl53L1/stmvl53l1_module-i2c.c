...
#if 0
static int stmvl53l1_read_chip_id(struct i2c_data *data)
{
	struct i2c_client *client = (struct i2c_client *) data->client;
	uint8_t buffer[2];
	uint8_t productId = 0;
	...
	if (rc != 2) {
		vl53l1_wanrmsg("fail to read vl53l1 chipID\n");
	} else {
		vl53l1_wanrmsg("success to read vl53l1 chipID, 0x010F:0x%x\n", productId);
	}
