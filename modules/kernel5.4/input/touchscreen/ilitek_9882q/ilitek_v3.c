...
static struct workqueue_struct *ili_fwu_wq;
...
void Ilitek_Update_Firmware(struct work_struct *work)
{
	ili_fw_upgrade_handler(NULL);
}
...
#endif
	ili_fwu_wq = alloc_workqueue("ili_fwu_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	if (!ili_fwu_wq) {
		ILI_ERR("ili_fwu_wq create workqueue failed\n");
	} else {
		INIT_DELAYED_WORK(&ilits->ili_fwu_work, Ilitek_Update_Firmware);
		queue_delayed_work(ili_fwu_wq, &ilits->ili_fwu_work, msecs_to_jiffies(14000));
	}
#else
#if (TDDI_INTERFACE == BUS_I2C && ENABLE_CASCADE)
	ili_ice_mode_ctrl_by_mode(DISABLE, OFF, BOTH);
...
void ili_dev_remove(bool flag)
{
	...
	if (ili_fwu_wq != NULL) {
		cancel_delayed_work_sync(&ilits->ili_fwu_work);
		flush_workqueue(ili_fwu_wq);
		destroy_workqueue(ili_fwu_wq);
	}
