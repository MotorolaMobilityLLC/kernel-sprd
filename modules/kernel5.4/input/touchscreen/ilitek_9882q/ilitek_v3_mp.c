...
static int open_test_cap(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		if (ipio_strcmp(tItems[index].desp, "open test_c (openx enable)") == 0) {
...
static int mp_show_result(bool lcm_on)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (ipio_strcmp(tItems[i].desp, "open test_c") == 0 || ipio_strcmp(tItems[i].desp, "open test_c (openx enable)") == 0) {
...
#else
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s.csv", CSV_LCM_DEF_PATH, get_date_time_str(), ret_fail_name);
#endif
		} else {
#if LINUX_VERSION_CODE <= KERNEL_VERSION(4, 14, 0)
...
#else
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s.csv", CSV_LCM_DEF_PATH, get_date_time_str(), ret_pass_name);
#endif
		} else {
#if LINUX_VERSION_CODE <= KERNEL_VERSION(4, 14, 0)
