...
static int ilitek_proc_mp_result_show(struct seq_file *m, void *v)
{
	...
	if (m->size < strlen(ilits->mp_result)) {
		ILI_INFO("size = %d, mp_result = %d\n Unable to print all data\n",
			m->size, strlen(ilits->mp_result));
	}

	seq_printf(m, "%s", ilits->mp_result);
	//ipio_vfree((void **)&(ilits->mp_result));
	return 0;
}
#endif
#endif
...
static ssize_t ilitek_node_mp_lcm_on_test_read_bak(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	int ret = 0;
	char self_test_buf[1] = {0};
	...
	if (copy_to_user((char *)buff, self_test_buf, 1))
		ILI_ERR("Failed to copy data to user space\n");
...
int ili_tp_data_mode_ctrl(u8 *cmd)
{
	...
	case DEBUG_LITE_PEN:
		if (ilits->PenType == POSITION_PEN_TYPE_ON) {
			...
			ILI_INFO("no support debug lite pen\n");
		}
		break;
...
int get_tp_gesture_status(int items){
...
EXPORT_SYMBOL(get_tp_gesture_status);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 6, 0)
static struct proc_ops proc_mp_lcm_on_test_fops_bak = {
	.proc_read = ilitek_node_mp_lcm_on_test_read_bak,
};
#else
static struct file_operations proc_mp_lcm_on_test_fops_bak = {
	.read = ilitek_node_mp_lcm_on_test_read_bak,
};
#endif
...
proc_node iliproc[] = {
	...
	{"tp_selftest", NULL, &proc_mp_lcm_on_test_fops_bak, false},
...
void ili_node_init(void)
{
	...
	for (; i < ARRAY_SIZE(iliproc); i++) {
		if( strncmp("tp_selftest",iliproc[i].name,strlen("tp_selftest")) == 0){
			iliproc[i].node = proc_create(iliproc[i].name, 0644, NULL, iliproc[i].fops);
		}
		else{
