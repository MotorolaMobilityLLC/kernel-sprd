...
static struct proc_dir_entry *TP_mt_entry_self_test = NULL;
...
static int32_t c_tp_show_self_test(struct seq_file *m, void *v)
{
	if (ts->pen_support) {
		if ((TestResult_Short == 0) && (TestResult_Open == 0) &&
			(TestResult_FW_Rawdata == 0) && (TestResult_FW_CC == 0) &&
...
const struct seq_operations tp_mt_selftest_ops = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
	.show   = c_tp_show_self_test
};
...
static int32_t tp_mt_selftest_open(struct inode *inode, struct file *file)
{
	struct device_node *np = ts->client->dev.of_node;
	unsigned char mpcriteria[32] = {0};	//novatek-mp-criteria-default
	...
	return seq_open(file, &tp_mt_selftest_ops);
}

#ifdef HAVE_PROC_OPS
static const struct proc_ops tp_mp_self_test = {
	.proc_open = tp_mt_selftest_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = seq_release,
};
#else
static const struct file_operations tp_mp_self_test = {
	.owner = THIS_MODULE,
	.open = tp_mt_selftest_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
int32_t nvt_mp_proc_init(void)
{
	...
	TP_mt_entry_self_test = proc_create("tp_selftest", 0444, NULL, &tp_mp_self_test);
	...
	if (TP_mt_entry_self_test == NULL) {
		...
		if(nvt_mp_buffer_init()) {
			...
			NVT_LOG("create /proc/tp_self_test Succeeded!\n");
		}
		return 0;
	}
}
...
void nvt_mp_proc_deinit(void)
{
	...
	if (TP_mt_entry_self_test != NULL) {
		remove_proc_entry("tp_self_test", NULL);
		TP_mt_entry_self_test = NULL;
		NVT_LOG("Removed /proc/%s\n", "tp_self_test");
	}
}
