...
int focaltech_read_id(void)
{
	unsigned char chip = 0, module = 0;
	bool err = false;
	err |= (1 != ts_reg_read(REG_CHIP_ID, &chip, 1));
	err |= (1 != ts_reg_read(REG_MODULE_ID, &module, 1));
	if(err || chip != 0x54 || module != 0x87 ){
		pr_err("failed to read chip id or chip id not match");
		return 0;
	}
	return 0x54;
