...
static int dvfs_gov_get_target(struct devfreq *devfreq,
    ...
    if (module->dvfs_enable) {
        ...
        *freq = adjusted_freq;
        ...
        pr_err("dcam01_dvfs_gov, %s: dvfs unenable", __func__);
    if(*freq < 307200000) {
        *freq = 307200000;
        pr_info("dcam01_dvfs_gov, %s: dcam clk too low, change freq to %lu", __func__, *freq);
    }
    pr_info("dcam01_dvfs_gov, %s: *freq:%lu", __func__, *freq);
