...
static int ip_auto_tune_en(struct devfreq *devfreq, unsigned long dvfs_eb) {
	...
	if (dvfs_eb)
		...
		dfs_en_reg | BIT_DCAM0_1_AXI_AUTO_TUNE_EN);
	else
		DVFS_REG_WR(REG_MM_DVFS_AHB_MM_AUTO_TUNE_EN_CTRL,
		dfs_en_reg & (~BIT_DCAM0_1_AXI_AUTO_TUNE_EN));
...
static int get_ip_dvfs_table(struct devfreq *devfreq,
    ...
    for (i = 0; i < 8; i++) {
        ...
        dvfs_table[i].dcam_axi_index = dcam0_1_axi_dvfs_config_table[i].dcam_axi_index;
        dvfs_table[i].mm_mtx_index = dcam0_1_axi_dvfs_config_table[i].mm_mtx_index;
