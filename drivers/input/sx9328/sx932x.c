...
static ssize_t sx932x_delay_store(struct device *dev,
                        struct device_attribute *attr, const char *buf, size_t count)
{       
        pr_info("[SX932x]: %s \n",__func__);
...
static ssize_t sx932x_sensor_hub_store(struct device *dev,
                        struct device_attribute *attr, const char *buf, size_t count)
{       
        pr_info("[SX932x]: %s \n",__func__);
...
static DEVICE_ATTR(delay,  0664, NULL,sx932x_delay_store);
static DEVICE_ATTR(batch,  0664, NULL,sx932x_batch_store);
static DEVICE_ATTR(flush,  0664, NULL,sx932x_flush_store);
static DEVICE_ATTR(sensor_hub,  0664, NULL,sx932x_sensor_hub_store);
...
static void touchProcess(psx93XX_t this)
{
	...
	if (this && (pDevice = this->pDevice))
	{
		...
		input_report_abs(input, ABS_RX, i);
                input_report_abs(input, ABS_RY, 0x99);
		input_sync(input);
	...
	if (this){
		...
		* (1->NIRQ=0, 0->NIRQ=1) */
		...
		if (pDevice){
			...
			input_set_abs_params(input, ABS_RX, 0, 65535, 0, 0);
        		input_set_abs_params(input, ABS_RY, 0, 65535, 0, 0);
