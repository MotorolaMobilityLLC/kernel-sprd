...
static int fpc_dts_irq_init(struct fpc_data *fpc)
{
	...
	if (error) {
        fpsensor_log(ERROR_LOG,"setup fpsensor irq gpio for input failed!error[%d]\n", error);
        return -EINVAL;
    }

    node = of_find_compatible_node(NULL, NULL,  "sprd,fingerprint-fpsensor");
    if ( node) {
...
static struct of_device_id fpc_of_match[] = {
    { .compatible = "sprd,fingerprint-fpc1020", },
    {}
};
