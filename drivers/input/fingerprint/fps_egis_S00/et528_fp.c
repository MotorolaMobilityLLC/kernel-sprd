...
#ifdef CONFIG_COMPAT
static long egisfp_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
int egisfp_parse_dt(struct egisfp_dev_t *egis_dev)
{
	...
	INFO_PRINT(" %s : start \n", __func__);
        ...
        power_node = of_find_compatible_node(NULL, NULL, "egis,fingerprint");
	if (power_node)
	{
		egis_dev->ctrl_power = of_property_read_bool(power_node, "fp-ctrl-power");
		if (egis_dev->ctrl_power)
		{
			egis_dev->pwr_by_gpio = of_property_read_bool(power_node, "fp-gpio-vcc-enable");
			if (egis_dev->pwr_by_gpio)
			{
				egis_dev->vcc_33v_Pin = of_get_named_gpio(power_node, "egistec,gpio_vcc_en", 0);
			...
			{
				ret = of_property_read_u32_array(power_node, "egis-fp,vcc-voltage", voltage_supply, 2);
				if (ret < 0)
				{
                ...
                {
                    ERROR_PRINT(" %s : ctrl_power property is null for power_node \n", __func__);
                }
                node = of_find_compatible_node(NULL, NULL, EGIS_COMPATIBLE_NODE);
		...
		egis_dev->irqPin = of_get_named_gpio(node, "fpsensor,eint-gpio", 0);
