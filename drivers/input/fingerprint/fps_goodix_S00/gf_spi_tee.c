...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
    ...
    if (node) {
        ...
        if (ret < 0) {
    		dev_err(gf_dev->device,  "[ERROR] failed to get fpsensor,reset-gpio.\n");
        return ret;
        } 
    	gf_dev->reset_gpio = ret;
        ret = devm_gpio_request(dev, gf_dev->reset_gpio, "fpsensor,reset-gpio");
        if (ret) {
			dev_err(gf_dev->device,  "[ERROR] failed to request fpsensor,reset-gpio.\n");
            return ret;
        }
        ret = of_get_named_gpio(node, "fpsensor,eint-gpio", 0);
        if (ret < 0) {
            dev_err(gf_dev->device,  "[ERROR] failed to get fpsensor,eint-gpio.\n");
            return ret;
        } 
        gf_dev->irq_gpio = ret;
        gf_debug(DEBUG_LOG, "fpsensor,eint-gpio =%d, fpsensor,reset-gpio = %d\n", gf_dev->irq_gpio, gf_dev->reset_gpio);
        gpio_direction_output(gf_dev->reset_gpio, 1);
      //  gpio_direction_output(gf_dev->irq_gpio, 0);
    } else {
		pr_err("[error] get goodix  gpio fail\n");
        ret = -ENODEV;
        return ret;
	}
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_dev>reset_gpio = of_get_named_gpio(node,e"gx, gpio_reset", 0);
...
static void gf_irq_gpio_cfg(struct gf_device *gf_dev)
{
	...
	error = gpio_request(gf_dev->irq_gpio, "fpsensor,eint-gpio");
	if (error) {
		dev_err(gf_dev->device,  "[ERROR] fpsensor,eint-gpio request failed.\n");
		return;
	}
...
static int __init gf_init(void)
{
	...
	gf_debug(ERR_LOG, "%s, the value of status is %d.\n", __func__,status);
	if (status < 0) {
