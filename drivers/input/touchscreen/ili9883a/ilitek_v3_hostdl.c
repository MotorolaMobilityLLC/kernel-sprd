...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	if (CTPM_FW[22] == 0xFF && CTPM_FW[23] == 0xFF &&
		...
		ILI_ERR("Invaild ILI format, abort!, 22=0x%x, 23=0x%x, 24=0x%x, 25=0x%x\n",
                CTPM_FW[22], CTPM_FW[23], CTPM_FW[24], CTPM_FW[25]);
		return -EINVAL;
	}
...
int ili_fw_upgrade(int op)
{
	...
	if (!ilits->eng_flow) {
		do {
			...
			ILI_ERR("Upgrade failed(engineer), do retry!\n");
		} while (--retry > 0);
	...
	do {
		...
		ILI_ERR("Upgrade failed(normal), do retry!\n");
	} while (--retry > 0);
