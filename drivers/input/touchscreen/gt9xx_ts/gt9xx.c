...
#endif
static u8 g_chipid[4];
static u16 g_fwversion;
static u8 g_vendorid;
...
{
    u8 nbuf[2];
	gtp_i2c_read_dbl_check(i2c_connect_client, GTP_REG_CONFIG_DATA,&nbuf[0], 1);
	g_fwversion = nbuf[0];
	return snprintf(buf, PAGE_SIZE, "gt9xx::chipid(0x%c%c%c%c)::fwversion(0x%0x.0x%x)\n",
		   g_chipid[0],g_chipid[1],g_chipid[2],g_chipid[3], g_vendorid,g_fwversion);
}
...
/*	if (is_have_tp) {
	...
	if(device_create_file(tp_cmd_dev, &dev_attr_tp_version) < 0) // /sys/class/mtk-tpd/device/tp_version
	{
		printk("Failed to create device file(%s)!\n", dev_attr_tp_version.attr.name);
	}
//	if(device_create_file(tp_cmd_dev, &dev_attr_tp_update) < 0) // /sys/class/mtk-tpd/device/tp_update
//	{
//		printk("Failed to create device file(%s)!\n", dev_attr_tp_update.attr.name);
//	}
#endif
	gtp_esd_init(ts);
	gtp_esd_on(ts);
