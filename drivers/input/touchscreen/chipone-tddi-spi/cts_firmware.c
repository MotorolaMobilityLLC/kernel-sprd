...
#define NUM_DRIVER_BUILTIN_FIRMWARE ARRAY_SIZE(cts_driver_builtin_firmwares_easyquick_608)
...
const struct cts_firmware *cts_request_driver_builtin_firmware_by_name(const char *name)
{
    ...
    if (LCM_INFO_EASYQUICK_608 == g_lcm_info_flag){
        ...
        firmware = cts_driver_builtin_firmwares_hlt;
    }
...
#define MATCH_FWID(firmware, fwid) \
    ...
    //firmware = cts_driver_builtin_firmwares;
    for (i = 0; i < ARRAY_SIZE(cts_driver_builtin_firmwares_easyquick_608); i++, firmware++) {
