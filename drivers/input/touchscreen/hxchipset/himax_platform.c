...
#if RESUME_SPRD
int himax_sleep_handler(int mode)
{
	int ret = 0;
	printk(KERN_ERR "ontim --->%s(%d) mode:%d\n", __func__, __LINE__, mode);
	switch (mode) {
		case TP_SUSPEND:
			printk(KERN_ERR "%s(%d) TP suspend start\n", __func__, __LINE__);
			himax_chip_common_suspend(private_ts);
			private_ts->suspended = true;
			printk(KERN_ERR "%s(%d) TP suspend end\n", __func__, __LINE__);
		break;
		case TP_RESUME:
			printk(KERN_ERR "%s(%d) TP resume start\n", __func__, __LINE__);
			himax_chip_common_resume(private_ts);
			private_ts->suspended = false;
			printk(KERN_ERR "%s(%d) TP resume end\n", __func__, __LINE__);
		break;
		default:
		printk(KERN_ERR "Unknown sleep mode:%d\n", mode);
		ret = -EINVAL;
		break;
	}
	return ret;
...
{
	return sprintf(buf, "%s\n", private_ts->suspended ? "true" : "false");
}
static ssize_t ts_suspend_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	printk(KERN_ERR "ontim --->%s(%d), buf[0]:%c, suspend(no use):%d\n", __func__, __LINE__, buf[0], private_ts->suspended);
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	ret = spi_setup(spi);
	if (ret < 0) {
		printk(KERN_ERR "Himax Failed to perform SPI setup\n");
	}
	...
	if (sysfs_create_link(NULL, &ts->dev->kobj, "touchscreen") < 0)
		printk(KERN_ERR "Failed to create link!\n");
#endif
...
{
	printk(KERN_ERR "Himax common touch panel driver init 123!\n");
	printk(KERN_ERR "Himax check double loading\n");
	if (g_mmi_refcnt++ > 0) {
