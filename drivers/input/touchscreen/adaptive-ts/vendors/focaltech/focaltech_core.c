...
struct ft_vendor_id_t vendor_id_map[]={
    {.type = 0x20,
     .vendor_ids = {0xF0},
    },
    {.type = 0x19,
...
void fts_set_chip_info(struct fts_ts_data *ts_data)
{
    if(ts_data->ic_info.vendor_id == 0xf0)
    {    
        printk("[TS] fts_set_chip_info:_FT8722\n");
        ts_data->ic_info.is_incell = ((_FT8722 & FLAGBIT(FLAG_IDC_BIT)) == FLAGBIT(FLAG_IDC_BIT));
        ts_data->ic_info.hid_supported = ((_FT8722 & FLAGBIT(FLAG_HID_BIT)) == FLAGBIT(FLAG_HID_BIT));
    }
    else if(ts_data->ic_info.vendor_id == 0xe0)
    {
#if defined(CONFIG_TOUCHSCREEN_ADAPTIVE_FT8006AB)
        printk("[TS] fts_set_chip_info:_FT8006S_AB\n");
        ts_data->ic_info.is_incell = ((_FT8006S_AB  & FLAGBIT(FLAG_IDC_BIT)) == FLAGBIT(FLAG_IDC_BIT));;
        ts_data->ic_info.hid_supported = ((_FT8006S_AB & FLAGBIT(FLAG_HID_BIT)) == FLAGBIT(FLAG_HID_BIT));
#else
        printk("[TS] fts_set_chip_info:_FT8006S_AA\n");
        ts_data->ic_info.is_incell = ((_FT8006S_AA & FLAGBIT(FLAG_IDC_BIT)) == FLAGBIT(FLAG_IDC_BIT));;
        ts_data->ic_info.hid_supported = ((_FT8006S_AA & FLAGBIT(FLAG_HID_BIT)) == FLAGBIT(FLAG_HID_BIT));
#endif
    }
    
    FTS_INFO("is_incell[%d] , hid_supported[%d]\n" , ts_data->ic_info.is_incell ,ts_data->ic_info.hid_supported );

}

int fts_get_bootic_information(struct fts_ts_data *ts_data)
{
    int ret = 0;
    u8 chip_id[2] = { 0 };
...
int fts_match_vendor_id(u8 vendor_id , u8 type)
{
    int i , j ;
    u32 vendorid_entries = sizeof(vendor_id_map) / sizeof(struct ft_vendor_id_t);
    for (i = 0; i < vendorid_entries; i++) {
        if(vendor_id_map[i].type == type)
        {
            for(j = 0 ; j < FTS_MAX_VENDOR_IDS ; j++)
            {
                if(vendor_id_map[i].vendor_ids[j] == 0)
                    break;
                if(vendor_id_map[i].vendor_ids[j] == vendor_id)
                    return 0 ;
            }
            break ; 
        }
...
int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    do {
        ...
        if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
            ...
            if (!ret)
           {
                ...
                if((ret < 0) || (0x0 == vendor_id))
                {
                    FTS_DEBUG("vendor id read invalid, read:0x%02x",vendor_id);
                }
                else
                {
                    ret = fts_match_vendor_id(vendor_id , ts_data->ic_info.ids.type);
                    if(ret < 0)
                    {
                        FTS_INFO("vendor id no match:id[0x%x] tyep[0x%x]\n" ,
                            vendor_id , ts_data->ic_info.ids.type);
                        return ret ;
                    }
                    else
                    {
                         FTS_INFO("vendor id match:id[0x%x] tyep[0x%x]\n" ,
                            vendor_id , ts_data->ic_info.ids.type);
                    }
                        
                    ts_data->ic_info.vendor_id = vendor_id ;
                    fts_set_chip_info(ts_data);
                    break;
                }
           }
            else
    ...
    FTS_INFO("is_incell[%d] , hid_supported[%d]\n" , ts_data->ic_info.is_incell ,ts_data->ic_info.hid_supported );
    FTS_INFO("get ic information, chip id = 0x%02x%02x(cid type=0x%x) vendor_id = 0x%02x",
             ts_data->ic_info.ids.chip_idh, ts_data->ic_info.ids.chip_idl,
             ts_data->ic_info.cid.type , ts_data->ic_info.vendor_id);
...
int fts_report_buffer_init(struct fts_ts_data *ts_data)
{
    ...
    events_num = point_num * sizeof(struct fts_ts_event);
    ts_data->events = (struct fts_ts_event *)kzalloc(events_num, GFP_KERNEL);
    if (!ts_data->events) {
