...
struct fts_upgrade_match upgrade_match_list[] ={
{0xF0 , 0x20},
...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg)
{
    ...
    if (fwvalid) {
        ...
        if(upg->ts_data->force_upgrade == 1)
        {
            FTS_INFO("force_upgrade\n");
            return true;
        }
        else
        {
...
#endif
static int fts_get_tpd_firmware(struct fts_upgrade *upg)
{
	u8 *fw_data = NULL;
	struct upgrade_module *upg_m = NULL;
	...
	ntv_dts_data.tpd_firmware_size = kmalloc(upg->ts_data->vendor_nums*sizeof(u32), GFP_KERNEL);
	if(ntv_dts_data.tpd_firmware_size == NULL){
		FTS_ERROR("tpd_firmware_size kmalloc is not found\n");
		return -1;
	}

	of_property_read_u32_array(upg->ts_data->update_node, "tp_fw_buf_size", ntv_dts_data.tpd_firmware_size, upg->ts_data->vendor_nums);

    upg_m = kmalloc(sizeof(struct upgrade_module), GFP_KERNEL);
	if(upg_m == NULL){
		FTS_ERROR("fw_entry malloc is not found\n");
		return -1;
	}

	upg_m->fw_len = ntv_dts_data.tpd_firmware_size[upg->ts_data->vendor_num];
        //fw_buf->data = kmalloc(fw_buf->size, GFP_KERNEL);
	fw_data = kmalloc(upg_m->fw_len, GFP_KERNEL);
	FTS_INFO("upg_m->fw_len=%d\n", upg_m->fw_len);
	if(fw_data == NULL){
		FTS_ERROR("fts_fw_entry->data malloc is not found\n");
		return -1;
	}

	sprintf(nvt_cfgs_grp, "tp_fw_buf%d", upg->ts_data->vendor_num);
	of_property_read_u8_array(upg->ts_data->update_node, nvt_cfgs_grp, fw_data, ntv_dts_data.tpd_firmware_size[upg->ts_data->vendor_num]);

	upg_m->fw_file = fw_data;
	upg_m->id = 1 ;
	upg->module_info = upg_m;
    FTS_INFO("end...\n");
	return 0;
}
...
int fts_fw_upgrade(void)
{
    ...
    {
		FTS_INFO("fts_fw_upgrade[%d]\n" ,fw_ver_in_tp);
	}

	memset(g_pdata->firmwork_version, 0x00, sizeof(g_pdata->firmwork_version));
	sprintf(g_pdata->firmwork_version, "0x%02x", fw_ver_in_tp);
    upg->ts_data->fw_loading = 0;
	return ret ;
}
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    int upgrade_match_count =  sizeof(upgrade_match_list) / sizeof(upgrade_match_list[0]);
    ...
    if(ts_data->force_upgrade == 1)
    {
        for(i = 0 ; i < upgrade_match_count ; i++)
        {
            if(ts_data->vendor_id == upgrade_match_list[i].vendor_id)
                ic_stype = upgrade_match_list[i].stype ;
        }
    }
    else
