...
/*****************************************************************************
*
* File Name: focaltech_upgrade_ft8722.c
*
...
*****************************************************************************/
u8 pb_file_ft8722[] = {
#include "../include/pramboot/FT8722_Pramboot_V1.1_20201022.i"
};
...
static int fts_ft8722_get_hlic_ver(u8 *initcode)
{
...
static int fts_ft8722_upgrade_mode(enum FW_FLASH_MODE mode, u8 *buf, u32 len)
{
    int ret = 0;
    u32 start_addr = 0;
    u8 cmd[4] = { 0 };
    ...
    if (upgrade_func_ft8722.appoff_handle_in_ic) {
        start_addr = 0; /* offset handle in pramboot */
    } else {
        start_addr = upgrade_func_ft8722.appoff;
    }
...
/************************************************************************
* Name: fts_ft8722_upgrade
* Brief:
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_ft8722_upgrade(u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 app_len = 0;
    ...
    app_len = len - upgrade_func_ft8722.appoff;
    tmpbuf = buf + upgrade_func_ft8722.appoff;
    ret = fts_ft8722_upgrade_mode(FLASH_MODE_APP, tmpbuf, app_len);
    if (ret < 0) {
...
static int fts_ft8722_lic_upgrade(u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 lic_len = 0;
    ...
    ret = fts_ft8722_upgrade_mode(FLASH_MODE_LIC, tmpbuf, lic_len);
    if (ret < 0) {
...
struct upgrade_func upgrade_func_ft8722 = {
    .ctype = {0x20},
    .fwveroff = 0x210E,
    .fwcfgoff = 0x1000,
    .appoff = 0x2000,
    ...
    .pramboot = pb_file_ft8722,
    .pb_length = sizeof(pb_file_ft8722),
    .pram_ecc_check_mode = ECC_CHECK_MODE_CRC16,
    .new_return_value_from_ic = true,
    .hid_supported = false,
    .upgrade = fts_ft8722_upgrade,
    .get_hlic_ver = fts_ft8722_get_hlic_ver,
    .lic_upgrade = fts_ft8722_lic_upgrade,
};
