...
#define FTS_CHIP_TYPE_MAPPING {{0x20, 0x87, 0x22, 0x87, 0x22, 0x87, 0xA2, 0x00, 0x00},\
                               {0x19, 0x86, 0x32, 0x86, 0x32, 0x86, 0xC2, 0x00, 0x00},\
                                ...
                                {0x1D, 0x80, 0x16, 0x86, 0x32, 0x86, 0xC2, 0x00, 0x00},\
                                {0x1F, 0x80, 0x17, 0x80, 0x17, 0x86, 0xC2, 0x00, 0x00},\
                              }
...
struct ft_vendor_id_t{
    u16 type;
    u8 vendor_ids[FTS_MAX_VENDOR_IDS];
};
...
#define FTS_DEBUG(fmt, args...) do { \
    printk(KERN_ERR"[TS][FTS_TS]%s:"fmt"\n", __func__, ##args); \
} while (0)

#define FTS_FUNC_ENTER() do { \
    printk(KERN_ERR"[TS][FTS_TS]%s: Enter\n", __func__); \
} while (0)

#define FTS_FUNC_EXIT() do { \
    printk(KERN_ERR"[TS][FTS_TS]%s: Exit(%d)\n", __func__, __LINE__); \
} while (0)
...
#define FTS_INFO(fmt, args...) do { \
    printk(KERN_ERR "[TS][FTS_TS/I]%s:"fmt"\n", __func__, ##args); \
} while (0)

#define FTS_ERROR(fmt, args...) do { \
    printk(KERN_ERR "[TS][FTS_TS/E]%s:"fmt"\n", __func__, ##args); \
} while (0)
