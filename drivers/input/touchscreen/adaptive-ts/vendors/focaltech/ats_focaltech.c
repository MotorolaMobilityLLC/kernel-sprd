...
int  fts_upgrade_init(void){
	...
	fts_data->update_node = of_get_child_by_name(p_node, "ats_ft8722");
	of_property_read_u32(fts_data->update_node, "tp_vendor_num", &fts_data->vendor_nums);
    ...
    if(fts_data->force_upgrade == 1)
    {
       vendor_id = fts_data->vendor_id ;
    }
    else
    {
        vendor_id = fts_data->ic_info.vendor_id ;
    }

    for(i=0; i <  fts_data->vendor_nums; i++ ){
    	if (tpd_vendor_id[i] == vendor_id){
    ...
    if(fts_data->ic_info.ids.chip_idh == 0x86 && fts_data->ic_info.ids.chip_idl == 0x32)
    {
        sprintf(g_pdata->chip_name, "ft8006s-aa");
    }
    else if(fts_data->ic_info.ids.chip_idh == 0x86 && fts_data->ic_info.ids.chip_idl == 0x42)
    {
        sprintf(g_pdata->chip_name, "ft8006s-ab");
    }
    else
    {
...
static void fts_custom_initialization(void)
{
	int ret = 0;
    u8 vendor_id ;
    if(g_pdata->vendor_cfg->match_status == LCDNAME_MATCH)
    {
        vendor_id = g_pdata->vendor_cfg->vendorid ;
        fts_data->force_upgrade = 1 ;
        fts_data->vendor_id = vendor_id ;
        fts_data->ic_info.vendor_id = vendor_id ;
        fts_set_chip_info(fts_data);
        fts_get_bootic_information(fts_data);
        memset(g_pdata->chip_name, 0x00, sizeof(g_pdata->chip_name));
        if(fts_data->ic_info.ids.chip_idh == 0x86 && fts_data->ic_info.ids.chip_idl == 0x32)
        {
            sprintf(g_pdata->chip_name, "ft8006s-aa");
...
int ft8722_init(void)
{
	ts_register_controller(&FtsController);
	return 0;
}

void ft8722_exit(void)
{
