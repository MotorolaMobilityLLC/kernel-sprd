...
//TS_ERR("spi_read_write client=%x\n",client);
	switch (rw) {
		case NVTREAD:
			t.tx_buf = ts->xbuf;
			t.rx_buf = ts->rbuf;
...
static void nvt_read_fw_history(uint32_t fw_history_addr){
    ...
    nvt_spi_read(ts->client, buf, 64+1);   //read 64bytes history
    //print all data
...
*******************************************************/
unsigned int nvt_ts_check_chip_ver_trim(void)
{
	uint8_t buf[8] = {0};
	int32_t retry = 0;
	int32_t list = 0;
...
NVT_ERR("nvt_ts_check_chip_ver_trim!\n");
	if(init_first_is==1){
...
//同时使用这个TP时，由于ICNL9911C需要使用到复位引脚，而nt36525b不需要，但是复位连接到屏IC了，而屏一端有1.8V输出，CPU为0，所以开机后要设置为CPU输出为高即可，否则会有漏电现象
...
int  nt36xxx_upgrade_init(void)
{
	...
	of_property_read_u32(ts->update_node, "tp_vendor_num", &ts->vendor_nums);
	...
	of_property_read_u32_array(ts->update_node, "tp_upgrade_fw", tpd_firmware_update, ts->vendor_nums);
	of_property_read_u8_array(ts->update_node, "tp_vendor_id", tpd_vendor_id, ts->vendor_nums);
	...
	for(i=0; i <  ts->vendor_nums; i++ ){
		if (tpd_vendor_id[i] ==nvt_vendor_id){
			...
			of_property_read_string(ts->update_node, nvt_vendor_name, (char const **)&g_pdata->vendor_string);
...
static int nt36xxx_ps_resume(struct ts_data *pdata){
	...
	/*鍦═S澶勪簬浼戠湢妯″紡涓嬶紝濡傛灉鏈夊杩涜ps鎵撳紑鎿嶄綔锛屽湪杩涘叆鍞ら啋鍚庯紝闇�瑕佸湪姝ゅ瀵瑰浐浠朵笅鍙憄s         浣胯兘鍙傛暟*/
	if (ts_get_mode_ext(pdata, TSMODE_PS_STATUS)||ts_get_mode_ext(pdata, TSMODE_SENSORHUB_STATUS)){
