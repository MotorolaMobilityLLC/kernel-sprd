...
unsigned char tlsc6x_chip_name[MAX_CHIP_ID][20] =
	{"null", "tlsc6206a", "tlsc6306", "tlsc6206", "tlsc6324", "tlsc6332", "chsc6440","chsc6448","chsc6432","chsc6424"};
...
int tlsc6x_esd_check_work(void){
	...
	//如果ESD时I2C的SDA被拉低了，那么读取出来的值是0，ret的值也是0，而不是ret<0
...
void tlsc6x_esd_status_init(void){
	...
	{/* esd issue: i2c monitor thread */
		...
		TS_INFO("tlsc6x_esd_status init !!!");
	}
	...
	unsigned char buf[FT_ALL_DATA_LEN] = { 0 };
		...
		tssc_get_debug_info(g_tlsc6x_client,buf);
    		...
    		if (tlsc6x_i2c_read(g_tlsc6x_client, buf, 1, buf, FT_ALL_DATA_LEN) < 0) {
    			TS_ERR("failed to read data apk debug");
    			return -1;
    		}
        }
#else
...
static  int tlsc6x_gesture_readdata(struct ts_controller *c)
{
	int ret = -1;
	int gestrue_id = 0;
	u8 buf[4] = {0xd3, 0xd3};
