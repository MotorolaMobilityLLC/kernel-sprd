...
{
    if(cmd==0xa0 || cmd==0xa1)//ps onoff log
	cts_err("cts_send_command cmd=0x%x", cmd);
...
int cts_get_touchinfo_ext(const struct cts_device *cts_dev,
        struct cts_device_touch_info *touch_info)
{
	cts_dbg("Get touch info");
...
int cts_irq_handler(struct cts_device *cts_dev)//binhua
{
    int ret;
...
void cts_init_rtdata_with_normal_mode(struct cts_device *cts_dev)
{
...
int cts_probe_device_ext(struct cts_device *cts_dev)
{
    int  ret=0, retries = 0;
...
read_fwid:
    if (ret == 0) {
        ...
        ret = cts_get_fwid(cts_dev, &fwid);//binhua
        if (ret) {
