...
#include "ats_cts_sfctrl.h"
#include "ats_cts_spi_flash.h"
...
#include "ats_cts_builtin_firmware.h"
...
static int icnl9911x_get_tpd_firmware(struct cts_firmware **firmware_p)
{
	u8 *fw_data = NULL;
	u8 fw_name[] = "icnl9911x";
	struct cts_firmware *fw_buf = NULL;
	...
	ntv_dts_data.tpd_firmware_size = kmalloc(cts_data->pdata->vendor_nums*sizeof(u32), GFP_KERNEL);
	if(ntv_dts_data.tpd_firmware_size == NULL){
		cts_err("tpd_firmware_size kmalloc is not found\n");
		return -1;
	}

	of_property_read_u32_array(cts_data->pdata->update_node, "tp_fw_buf_size", ntv_dts_data.tpd_firmware_size, cts_data->pdata->vendor_nums);

        fw_buf = kmalloc(sizeof(struct cts_firmware), GFP_KERNEL);
	if(fw_buf == NULL){
		cts_err("fw_entry malloc is not found\n");
		return -1;
	}

	fw_buf->size = ntv_dts_data.tpd_firmware_size[cts_data->pdata->vendor_num];
        //fw_buf->data = kmalloc(fw_buf->size, GFP_KERNEL);
	fw_data = kmalloc(fw_buf->size, GFP_KERNEL);
	cts_info("fw_buf->size=%d\n", fw_buf->size);
	if(fw_data == NULL){
		cts_err("ili_fw_entry->data malloc is not found\n");
		return -1;
	}

	sprintf(nvt_cfgs_grp, "tp_fw_buf%d", cts_data->pdata->vendor_num);
	of_property_read_u8_array(cts_data->pdata->update_node, nvt_cfgs_grp, fw_data, ntv_dts_data.tpd_firmware_size[cts_data->pdata->vendor_num]);

	fw_buf->data = fw_data;
	fw_buf->name = kmalloc(strlen(fw_name)+1, GFP_KERNEL);
	sprintf(fw_buf->name, fw_name);
	fw_buf->hwid = CTS_DEV_HWID_ICNL9911C;
	fw_buf->fwid = CTS_DEV_FWID_INVALID;
        ...
        const char *filepath, u16 curr_version)
{
	...
	if(!g_fw_control_icnl){
		ret = icnl9911x_get_tpd_firmware(&firmware);
		if(ret!=0){
			cts_err(" failed. (%d)\n", ret);
		}
