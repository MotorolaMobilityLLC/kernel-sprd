...
#define SHORT_TEST_CSV_FILE 		"/data/local/tmp/ShortTest.csv"//"/sdcard/nt36xxx/ShortTest.csv"//"/data/local/tmp/ShortTest.csv"
...
static void nvt_enable_noise_collect(int32_t frame_num)
{
	...
	buf[3] = frame_num;
	buf[4] = 0x00;
	nvt_ctp_i2c_write(ts->client, I2C_FW_Address, buf, 5);
}
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		...
		nvt_ctp_i2c_read(ts->client, I2C_FW_Address, buf, IC_X_CFG_SIZE * 2 + 1);
	...
	nvt_ctp_i2c_read(ts->client, I2C_FW_Address, buf, IC_KEY_CFG_SIZE * 2 + 1);
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		...
		nvt_ctp_i2c_read(ts->client, I2C_FW_Address, buf, X_Channel * 2 + 1);
	...
	nvt_set_page(I2C_FW_Address, raw_btn_pipe_addr);
	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	nvt_ctp_i2c_read(ts->client, I2C_FW_Address, buf, Key_Channel * 2 + 1);
...
{
	NVT_proc_selftest_entry = proc_create("nvt_selftest", 0664, NULL, &nvt_selftest_fops);
