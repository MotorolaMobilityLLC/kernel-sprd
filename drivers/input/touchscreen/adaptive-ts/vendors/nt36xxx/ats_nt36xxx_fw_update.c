...
struct nvt_firmware *fw_entry = {0};//&__fw_entry;
static size_t fw_need_write_size = 0;

static int32_t nvt_get_fw_need_write_size(const struct nvt_firmware *fw_entry)
{
	int32_t i = 0;
...
int32_t Check_FW_Ver(void)
{
	...
	buf[1] = 0x00;
	buf[2] = 0x00;
	ret = nvt_ctp_i2c_read(ts->client, I2C_BLDR_Address, buf, 3);
	if (ret < 0) {
...
int32_t Resume_PD(void)
{
	...
	buf[0] = 0x00;
	buf[1] = 0xAB;
	ret = nvt_ctp_i2c_write(ts->client, I2C_HW_Address, buf, 2);
	if (ret < 0) {
	...
	while(1) {
		...
		buf[0] = 0x00;
		buf[1] = 0x00;
		ret = nvt_ctp_i2c_read(ts->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
...
int32_t Check_CheckSum(void)
{
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_need_write_size > (i * SIZE_64KB)) {
			...
			ret = nvt_ctp_i2c_write(ts->client, I2C_HW_Address, buf, 7);
			if (ret < 0) {
...
int32_t Init_BootLoader(void)
{
	...
	ret = nvt_ctp_i2c_write(ts->client, I2C_HW_Address, buf, 3);
	if (ret < 0) {
...
int32_t Erase_Flash(void)
{
	...
	while (1) {
		...
		buf[1] = 0x00;
		buf[2] = 0x00;
		ret = nvt_ctp_i2c_read(ts->client, I2C_HW_Address, buf, 3);
		if (ret < 0) {
	...
	for(i = 0; i < count; i++) {
		...
		ret = nvt_ctp_i2c_write(ts->client, I2C_HW_Address, buf, 5);
		if (ret < 0) {
...
int32_t Write_Flash(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		for (j = 0; j < min(fw_need_write_size - i * 256, (size_t)256); j += 32) {
			...
			ret = nvt_ctp_i2c_write(ts->client, I2C_BLDR_Address, buf, 33);
			if (ret < 0) {
		...
		buf[7] = tmpvalue;
		ret = nvt_ctp_i2c_write(ts->client, I2C_HW_Address, buf, 8);
		if (ret < 0) {
...
int32_t nvt_check_flash_end_flag(void)
{
	...
	ret = nvt_ctp_i2c_read(ts->client, I2C_BLDR_Address, buf, 6);
	if (ret < 0) {
...
int nvt_get_tpd_firmware(struct i2c_client *client)
{
	int i = 0;
	int ret1 = 0;
	...
	if (!node) {
		NVT_ERR("[tpd]%s can't find touch compatible custom node\n", __func__);
		return -ENODEV;
	}
	
	node = of_get_child_by_name(node, "ats_nt36xxx");
	ret1 = of_property_read_u32(node, "tp_vendor_num", &ntv_dts_data.vendor_num);
	NVT_LOG("[tpd] tp_vendor_num:%d\n",ntv_dts_data.vendor_num);

	tpd_vendor_id = kmalloc(ntv_dts_data.vendor_num, GFP_KERNEL);
	if(tpd_vendor_id == NULL){
		NVT_ERR("tpd_vendor_id kmalloc is not found\n");
		return -1;
	}

	tpd_firmware_update = kmalloc(ntv_dts_data.vendor_num*sizeof(u32), GFP_KERNEL);
	if(tpd_firmware_update == NULL){
		NVT_ERR("is_upgrade_switch kmalloc is not found\n");
		return -1;
	}

	ntv_dts_data.tpd_firmware_size = kmalloc(ntv_dts_data.vendor_num*sizeof(u32), GFP_KERNEL);
	if(ntv_dts_data.tpd_firmware_size == NULL){
		NVT_ERR("tpd_firmware_size kmalloc is not found\n");
		return -1;
	}

	ret1 = of_property_read_u8_array(node, "tp_vendor_id", tpd_vendor_id, ntv_dts_data.vendor_num);
	//ret1 = of_property_read_u8_array(node->parent, "tp_upgrade_switch", tpd_firmware_update, ntv_dts_data.vendor_num);
	ret1 = of_property_read_u32_array(node, "tp_upgrade_fw", tpd_firmware_update, ntv_dts_data.vendor_num);
	ret1 = of_property_read_u32_array(node, "tp_fw_buf_size", ntv_dts_data.tpd_firmware_size, ntv_dts_data.vendor_num);	
	for(i=0; i<ntv_dts_data.vendor_num; i++){
		NVT_LOG("[tpd] tp_vendor_id[%d] = 0x%x\n",  i, tpd_vendor_id[i]);
		NVT_LOG("[tpd] tp_upgrade_fw[%x] = %d\n",  i, tpd_firmware_update[i]);
		NVT_LOG("[tpd] tp_fw_buf_size[%d] = %d\n",  i,  ntv_dts_data.tpd_firmware_size[i]);
	}

	if(lcd_moden_from_uboot==0){
		if(ntv_dts_data.vendor_num>1)//目前只有P651W项目有两个同时是nt36xxx的TP，其它项目这个值都是1
			nvt_vendor_id = 0xD1;
		else
			nvt_vendor_id = 0x01;
	}
        ...
        fw_entry = kmalloc(sizeof(struct nvt_firmware), GFP_KERNEL);
	if(fw_entry == NULL){
		NVT_ERR("fw_entry malloc is not found\n");
		return -1;
	}

	NVT_LOG("[tpd] nvt_vendor_id=0x%x \n", nvt_vendor_id);

	for(i=0; i <  ntv_dts_data.vendor_num; i++ ){
		if (tpd_vendor_id[i] ==nvt_vendor_id){
		        fw_entry->data = kmalloc(ntv_dts_data.tpd_firmware_size[i], GFP_KERNEL);
			//fw_entry->data = vmalloc(ntv_dts_data.tpd_firmware_size[i]);
			if(fw_entry->data == NULL){
				NVT_ERR("fw_entry->data malloc is not found\n");
				return -1;
			}
			g_pdata->firmware_update_switch = tpd_firmware_update[i];

			sprintf(nvt_cfgs_grp, "tp_fw_buf%d", i);
			sprintf(nvt_vendor_name, "tp_vendor_name%d", i);
			ret1 = of_property_read_string(node, nvt_vendor_name, (char const **)&g_pdata->vendor_string);
			ret1 = of_property_read_u8_array(node, nvt_cfgs_grp, fw_entry->data, ntv_dts_data.tpd_firmware_size[i]);
			//NVT_LOG("nvt_cfgs_grp=%s, nvt_vendor_name=%s, vendor_string=%s\n", nvt_cfgs_grp, nvt_vendor_name, g_pdata->vendor_string);

			memset(g_pdata->chip_name, 0x00, sizeof(g_pdata->chip_name));
			memcpy(g_pdata->chip_name, ts_data->chipName, sizeof(ts_data->chipName));

			memset(g_pdata->firmwork_version, 0x00, sizeof(g_pdata->firmwork_version));
			sprintf(g_pdata->firmwork_version, "0x%02x", ts->fw_ver);

			//fw_need_write_size
			fw_entry->size = ntv_dts_data.tpd_firmware_size[i];
			if (nvt_get_fw_need_write_size(fw_entry)) {
				NVT_ERR("get fw need to write size fail!\n");
				return -EINVAL;
			}
		...
		if(i==ntv_dts_data.vendor_num-1)
			printk("[tpd] tpd_vendor_id:0x%x is not find error\n", nvt_vendor_id);
	}
	kfree(tpd_vendor_id);
	return 0;
}
...
int Boot_Update_Firmware(void)
{
...
#else
	nvt_get_tpd_firmware(ts->client);
#endif
	...
	if(g_pdata->firmware_update_switch){
		...
		NVT_LOG("force update firmware version ");
		Update_Firmware();
#else
