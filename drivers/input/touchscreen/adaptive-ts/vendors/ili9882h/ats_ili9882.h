...
({									\
	pr_info("[TS] ILITEK: (%s, %d): " fmt, __func__, __LINE__, ##arg);	\
})									\

#define ILI_ERR(fmt, arg...)						\
({									\
	pr_err("[TS] ILITEK: (%s, %d): " fmt, __func__, __LINE__, ##arg);	\
})									\
...
extern struct mutex ili_i2cwr_lock;
...
extern int ili_i2c_wrapper(u8 *txbuf, u32 wlen, u8 *rxbuf, u32 rlen, bool spi_irq, bool i2c_irq);
extern int ili_spi_wrapper(u8 *txbuf, u32 wlen, u8 *rxbuf, u32 rlen, bool spi_irq, bool i2c_irq);
extern void ili_ic_func_ctrl_plug(void);
#if defined(CONFIG_TOUCHSCREEN_ADAPTIVE_ILI9882H_SPI)
	extern int g_ili9882n_info;
#endif
