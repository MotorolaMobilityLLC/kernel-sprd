...
int ili_ic_proximity_resume(void)
{
	int ret=-1;
/*
	ret = ili_ic_func_ctrl("lpwg", 0x00);
	if (ret < 0) {
		ILI_ERR("Write lpwg cmd failed\n");
	}

	ret = ili_ic_check_busy(5, 10);
	if (ret < 0) {
		ILI_ERR("Check busy timeout during resume\n");
	}
*/
	...
	ILI_INFO("proximity_resume !!!\n");
#if defined( CONFIG_TOUCHSCREEN_ADAPTIVE_ILI9882H_SPI)
	ret = ilits->wrapper(wakeup, sizeof(wakeup), NULL, 0, OFF, OFF); 
#else
	ret = ilits->wrapper(&wakeup, sizeof(wakeup), NULL, 0, OFF, OFF); 
#endif

	if (ret < 0) {
		ILI_ERR("Write wakeup cmd failed\n");
	}
...
int ili_ic_func_ctrl(const char *name, int ctrl)
{
	...
	if((ctrl==1)&&(!ipio_strcmp(name, "proximity"))){//如果复位后需要下发命令，则要加这个延时，下发太快则会无效
		mdelay(90);
		ilits->tp_suspend = false;
	}

	if((ctrl==0x20)&&(!ipio_strcmp(name, "lpwg"))){
		ilits->prox_near = true;
	}
	else if((ctrl==0x01)&&(!ipio_strcmp(name, "sense"))){
...
void ili_ic_func_ctrl_reset(void) {
	...
	for (i = 0; i < FUNC_CTRL_NUM; i++) {
		if (func_ctrl[i].rec_state == ENABLE) {
			...
			if((!strcmp(func_ctrl[i].name, "plug"))
				&&(func_ctrl[i].rec_cmd==1 ||func_ctrl[i].rec_cmd==0))//usb control not need
				return;
...
void ili_ic_func_ctrl_plug(void)
{
	if (ili_ic_func_ctrl(func_ctrl[6].name, func_ctrl[6].rec_cmd) < 0)
		ILI_ERR("reset ic func ctrl %s failed\n", func_ctrl[6].name);
}
...
void ili_ic_get_pc_counter(int stat)
{
	...
	ILI_DBG("Read counter (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x\n",
...
int ili_ic_get_panel_info(void)
{
	...
	if (buf[0] != cmd) {
		...
		ilits->panel_wid = g_pdata->board->lcd_width;
		ilits->panel_hei = g_pdata->board->lcd_height;
		ilits->trans_xy = OFF;
	} else {
		ilits->panel_wid = g_pdata->board->lcd_width;//buf[1] << 8 | buf[2];
		ilits->panel_hei = g_pdata->board->lcd_height;//buf[3] << 8 | buf[4];
...
static void ilitek_tddi_ic_check_protocol_ver(u32 pver)
{
	...
	for (i = 0; i < PROTOCL_VER_NUM - 1; i++) {
		if (protocol_info[i].ver == pver) {
			...
			ILI_DBG("update protocol version = %x\n", ilits->protocol->ver);
			return;
		}
	}
...
int ili_ic_get_protocl_ver(void)
{
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf, ilits->protocol->pro_ver_len, ON, OFF)) {
		ILI_DBG("Read protocol version error\n");
		ret = -EINVAL;
		goto out;
	}
