...
int ili_touch_esd_gesture_iram(void)
{
	...
	ili_ic_func_ctrl("proximity", 0x01);
	ili_ic_func_ctrl("lpwg", 0x20);
	return ret;
...
void ili_report_ap_mode(u8 *buf, int len)
{
	...
	for (i = 0; i < g_pdata->board->max_touch_num/*MAX_TOUCH_NUM*/; i++) {
		if ((buf[(4 * i) + 1] == 0xFF) && (buf[(4 * i) + 2] == 0xFF)
			&& (buf[(4 * i) + 3] == 0xFF)) {
			if (MT_B_TYPE)
				ilits->curt_touch[i] = 0;
			ilits->points[i].pressed = 0;
			ilits->points[i].slot = i;
			continue;
		}
		...
		ilits->points[ilits->finger].x = touch_info[ilits->finger].x;
		ilits->points[ilits->finger].y = touch_info[ilits->finger].y;
		ilits->points[ilits->finger].pressed = 1;
		ilits->points[ilits->finger].slot = touch_info[ilits->finger].id;//MT_TOOL_FINGER   
		ilits->points[ilits->finger].pressure = touch_info[ilits->finger].pressure;//ABS_MT_PRESSURE   
		ilits->points[ilits->finger].touch_major = touch_info[ilits->finger].pressure;//ABS_MT_TOUCH_MAJOR   
		ILI_DBG("[TS] original x = %d, y = %d\n", xop, yop);
		ilits->finger++;
		if (MT_B_TYPE)
	...
	if(ilits->finger==0)
		ilits->finger = g_pdata->board->max_touch_num;
