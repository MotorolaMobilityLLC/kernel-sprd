 ...
 */
#include "firmware/ats_ili9882_fw.h"
...
//	ILI_INFO("Sleep Mode = %d\n", mode);
	...
	case TP_RESUME:
		...
		ilits->prox_near = false;//防止某些特殊状态下resume时没有清掉proximity状态，固件误入暗屏proximity导致TP无触发生
...
int ili_report_handler(void)
{
	...
	if (ret < 0) {
		ILI_DBG("Read report packet failed, ret = %d\n", ret);
		if (ret == DO_SPI_RECOVER) {
			ili_ic_get_pc_counter(DO_SPI_RECOVER);
			if (ilits->prox_near) {
				ILI_ERR("prox failed, doing prox recovery\n");
...
static void ili_update_tp_module_info(void)
{
	...
	if (module == 0 || ilits->md_fw_ili_size < ILI_FILE_HEADER) {
		ILI_DBG("Couldn't find any tp modules, applying default settings\n");
		ilits->md_name = "DEF";
	...
	ILI_DBG("Found %s module: ini path = %s, fw path = (%s, %s, %d)\n",
			ilits->md_name,
			ilits->md_ini_path,
...
int ili_tddi_init(void)
{
	...
	ilits->info_from_hex = ENABLE;//等于DISABLE是从IC读取固件版本号，等于ENABLE，是从DTSI读取固件版本号
#endif
	...
	ilits->ws = wakeup_source_register(&g_client->dev, "ili_wakelock");
	if (!ilits->ws)
