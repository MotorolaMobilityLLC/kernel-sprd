...
static int core_i2c_write(void *buf, int len)
{
	...
	struct i2c_msg msgs[] = {
		{
		 .addr = ilits->i2c->addr,
		 .flags = !I2C_M_RD,	/* write flag. */
		 .len = len,
		 .buf = txbuf,
		 },
	};

	mutex_lock(&ili_i2cwr_lock);
#if I2C_DMA_TRANSFER
	if (len > 8) {
...
int ili_i2c_wrapper(u8 *txbuf, u32 wlen, u8 *rxbuf, u32 rlen, bool spi_irq, bool i2c_irq)
{
	int ret = 0, operate = -1;
