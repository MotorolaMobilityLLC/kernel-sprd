...
static int ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	fbi[TAG1].mem_start = (fbi[TAG1].fix_mem_start != INT_MAX) ? fbi[TAG1].fix_mem_start : 0;
	...
	ILI_DBG("==== Gesture loader info ====\n");
	ILI_DBG("gesture move to ap addr => start = 0x%x, ap_end = 0x%x, ap_len = 0x%x\n", fbi[GESTURE].mem_start, ap_end, ap_len);
	ILI_DBG("gesture hex addr => start = 0x%x, gesture_end = 0x%x, gesture_len = 0x%x\n", ges_fw_start, ges_fw_end, fbi[GESTURE].len);
	ILI_DBG("=============================\n");
...
static int ili_get_tpd_firmware(struct spi_device *client)
{
	...
	ILI_INFO("ili_fw_entry->size=%d\n", ili_fw_entry->size);
	sprintf(nvt_cfgs_grp, "tp_fw_buf%d", ilits->vendor_num);
	of_property_read_u8_array(ilits->update_node, nvt_cfgs_grp, ili_fw_entry->data, ili_fw_entry->size);
...
int ili_fw_upgrade(int op)
{
	...
	if (!ilits->boot || ilits->force_fw_update || ERR_ALLOC_MEM(pfw)) {
		...
		if (ilitek_tdd_fw_hex_open(op, pfw) < 0) {
			...
			ili_get_tpd_firmware(ilits->spi);//binhua20200619
	...
	do {
		...
		if (ret == UPDATE_PASS){
			ILI_INFO("Update firmware success!\n");
			break;
		}
