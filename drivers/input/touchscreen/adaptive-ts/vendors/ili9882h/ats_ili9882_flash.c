...
static u8 *pfw;
static u8 *CTPM_FW;
struct ili_firmware *ili_fw_entry = {0};//&__fw_entry;
...
static void ilitek_tddi_flash_protect(bool enable)
{
	...
	case 0xC8:
		...
		break;
	default:
		ILI_DBG("Can't find flash id(0x%x), ignore protection\n", ilits->flash_mid);
		break;
	}
...
static int ilitek_tddi_flash_fw_crc_check(u8 *pfw, bool check_fw_ver)
{
	...
	if (check_fw_ver) {
		...
		if ((tfd.new_fw_cb > ilits->chip->fw_ver)||ilits->update_node) {
	...
	ILI_DBG("Flash FW is the same as targe file FW\n");
	return UPDATE_PASS;
}
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ILI_DBG("report_info_block : nReportByPixel = %d, nIsHostDownload = %d, nIsSPIICE = %d, nIsSPISLAVE = %d\n",
		ilits->rib.nReportByPixel, ilits->rib.nIsHostDownload, ilits->rib.nIsSPIICE, ilits->rib.nIsSPISLAVE);
	ILI_DBG("report_info_block : nIsI2C = %d, nReserved00 = %d, nReserved01 = %x, nReserved02 = %x,  nReserved03 = %x\n",
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	CTPM_FW = ili_fw_entry->data;//ilits->md_fw_ili;
	size = ili_fw_entry->size;//ilits->md_fw_ili_size;
	...
	ILI_DBG("Parsing ILI file, block num = %d, block mapping = %x\n", blk_num, blk_map);
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
	...
	case FILP_OPEN:
		...
		if (ERR_ALLOC_MEM(f)) {
			ILI_DBG("Failed to open the file at %ld\n", PTR_ERR(f));
			ret = -1;
			goto out;
		}
...
static int ili_get_tpd_firmware(struct i2c_client *client)
{
	...
	ntv_dts_data.tpd_firmware_size = kmalloc(ilits->vendor_nums*sizeof(u32), GFP_KERNEL);
	if(ntv_dts_data.tpd_firmware_size == NULL){
		ILI_ERR("tpd_firmware_size kmalloc is not found\n");
		return -1;
	}

	of_property_read_u32_array(ilits->update_node, "tp_fw_buf_size", ntv_dts_data.tpd_firmware_size, ilits->vendor_nums);

        ili_fw_entry = kmalloc(sizeof(struct ili_firmware), GFP_KERNEL);
	if(ili_fw_entry == NULL){
		ILI_ERR("ili_fw_entry malloc is not found\n");
		return -1;
	}

	ili_fw_entry->size = ntv_dts_data.tpd_firmware_size[ilits->vendor_num];
        ili_fw_entry->data = kmalloc(ili_fw_entry->size, GFP_KERNEL);
	//ili_fw_entry->data = vmalloc(ntv_dts_data.tpd_firmware_size[i]);
	if(ili_fw_entry->data == NULL){
		ILI_ERR("ili_fw_entry->data malloc is not found\n");
		return -1;
	}

	sprintf(nvt_cfgs_grp, "tp_fw_buf%d", ilits->vendor_num);
	of_property_read_u8_array(ilits->update_node, nvt_cfgs_grp, ili_fw_entry->data, ntv_dts_data.tpd_firmware_size[ilits->vendor_num]);
	//ILI_INFO("nvt_cfgs_grp=%s\n", nvt_cfgs_grp);
...
int ili_fw_upgrade(int op)
{
	...
	if (!ilits->boot || ilits->force_fw_update || ERR_ALLOC_MEM(pfw)) {
		if (ERR_ALLOC_MEM(pfw)) {
			ipio_vfree((void **)&pfw);
			pfw = vmalloc(MAX_HEX_FILE_SIZE * sizeof(u8));//160K
			if (ERR_ALLOC_MEM(pfw)) {
		...
		if (ilitek_tdd_fw_hex_open(op, pfw) < 0) {
			...
			ili_get_tpd_firmware(ilits->i2c);//binhua20200619
...
out:
	...
	ilits->info_from_hex = ENABLE;
#else
	ilits->info_from_hex = DISABLE;//等于DISABLE是从IC读取固件版本号，等于ENABLE，是从DTSI读取固件版本号
#endif
...
void ili_fw_read_flash_info(void)
{
	...
	if (i >= ARRAY_SIZE(flash_t)) {
		ILI_DBG("Not found flash id in tab, use default\n");
	...
	ILI_DBG("Flash MID = %x, Flash DEV_ID = %x\n", ilits->flash_mid, ilits->flash_devid);
	ILI_DBG("Flash program page = %d\n", ilits->program_page);
	ILI_DBG("Flash sector = %d\n", ilits->flash_sector);
