...
enum ts_event {
	...
	TSEVENT_HORIZONTAL_SCREEN_LEFT, /*Left(USB as reference) Horizontal screen mode */
	TSEVENT_HORIZONTAL_SCREEN_RIGHT,/*Right(USB as reference) Horizontal screen mode*/
	TSEVENT_VERTICAL_SCREEN,   /*Vertical screen mode*/
};
...
enum ts_gesture_status {
	TPG_OFF  = 0,
	TPG_ON,
};


struct msg_gesture_map {
	unsigned int enabled;
	unsigned char tp_gesture;
	unsigned char report_key;
	unsigned short reg;
	unsigned int bit;
};

enum vendor_match_status {
    NO_MATCH = 0,
	CHIPID_MATCH,
	LCDNAME_MATCH,
	FORCE_MATCH,
...
struct verdor_cfg{
    ...
    char vendor[16];
    char name[16];
    struct lcd_match_info *match_info;
    int match_info_count ;
    unsigned int vendorid;
    int (*read_chipid)(void);
    int (*verdor_init)(void);
    void (*verdor_exit)(void);
    int force_match ;
    enum vendor_match_status match_status ;
};
...
struct ts_data {
	...
	unsigned int width;
	unsigned int height;
	unsigned short gesture_enable;
	enum ts_gesture_status gesture_state;
	unsigned short tpd_prox_old_state;
	unsigned short tpd_prox_active;
	unsigned short touch_point;
	unsigned char ps_buf;
	unsigned short  usb_plug_state;
	...
	enum ts_pm_status tpm_status;
	enum ts_ps_status tps_status;
    struct verdor_cfg *vendor_cfg ;
	char chip_name[16];
	const char *vendor_string;
	uint8_t firmwork_version[16];
	unsigned int firmware_update_switch;
...
struct ts_controller {
    ...
    struct msg_gesture_map *msg_gestures_maps;
    const unsigned int msg_gestures_count;
	void (*custom_initialization)(void);
	void (*get_tp_info)(void);
	void (*ps_reset)(void);
	int (*ps_resume)(struct ts_data *);
	int (*ps_suspend)(struct ts_data *);
	void (*proximity_switch)(bool);
	void (*ps_irq_handler)(struct ts_data *);
	...
	int (*upgrade_status)(struct ts_controller *);
	unsigned char (*gesture_readdata)(struct ts_controller *);
    int (*gesture_config)(struct ts_controller *, int onoff, struct msg_gesture_map *map);
	void (*gesture_init)(struct ts_controller *);
	int (*gesture_exit)(struct ts_controller *);
	int (*gesture_suspend)(struct ts_controller *);
	int (*gesture_resume)(struct ts_controller *);
...
int ts_reg_write_fw(unsigned short reg, unsigned char *data,
		unsigned short length);
int ts_reg_read_fw(unsigned short reg, unsigned char *data,
		unsigned short length);
