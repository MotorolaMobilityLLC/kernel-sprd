...
static struct ip_dvfs_map_cfg map_table[] = {
	{0, V60, CLK_INDEX_256M, DPU_CLK256M},
	{1, V60, CLK_INDEX_307M2, DPU_CLK307M2},
...
static void get_dpu_dvfs_status(struct dpu_dvfs *dpu, struct ip_dvfs_status *dvfs_status)
{
	...
	dvfs_status->apsys_cur_volt =
		qogirn6lite_apsys_val_to_volt(reg->dpu_vsp_dvfs_voltage_dbg >> 12 & 0x7);
	dvfs_status->vsp_vote_volt =
		qogirn6lite_apsys_val_to_volt(reg->dpu_vsp_dvfs_voltage_dbg >> 6 & 0x7);
	dvfs_status->dpu_vote_volt =
		qogirn6lite_apsys_val_to_volt(reg->dpu_vsp_dvfs_voltage_dbg >> 3 & 0x7);
	dvfs_status->vdsp_vote_volt = "N/A";

	dvfs_status->vsp_cur_freq = "N/A";
	dvfs_status->dpu_cur_freq =
		qogirn6lite_dpu_val_to_freq(reg->dpu_vsp_vpu_dispc0_dvfs_cgm_cfg_dbg & 0x7);
...
const struct dpu_dvfs_ops qogirn6lite_dpu_dvfs_ops = {
