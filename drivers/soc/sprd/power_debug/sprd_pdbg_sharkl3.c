...
static void sharkl3_output_2nd_irq_source(void *pentry, u32 hw_irq_nr)
{
	...
	if (hw_irq_nr == AP_INTC_PMIC_INDEX)
		sc2721_output_irq_source(pdbg_sharkl3.pmic_regmap);
}
...
static int sprd_pdbg_sharkl3_probe(struct platform_device *pdev)
{
	...
	if (regmap_np)
		...
		pdev_pmic = NULL;
	if (pdev_pmic)
		pdbg_sharkl3.pmic_regmap = dev_get_regmap(
						pdev_pmic->dev.parent, NULL);
	else
		pdbg_sharkl3.pmic_regmap = NULL;

	if (!pdbg_sharkl3.pmic_regmap)
...
static int __init sprd_pdbg_sharkl3_driver_init(void)
{
	return platform_driver_register(&sprd_pdbg_sharkl3_driver);
}

static void __exit sprd_pdbg_sharkl3_driver_exit(void)
{
	platform_driver_unregister(&sprd_pdbg_sharkl3_driver);
}

late_initcall(sprd_pdbg_sharkl3_driver_init);
module_exit(sprd_pdbg_sharkl3_driver_exit);
