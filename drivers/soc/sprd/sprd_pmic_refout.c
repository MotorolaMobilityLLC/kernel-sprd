...
static struct pmic_refout *ump9622_refout = NULL;
...
static ssize_t pmic_refout_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (ump9622_refout == NULL)
		return sprintf(buf, "The pmic refout driver is not init\n");

	ret = regmap_read(ump9622_refout->regmap, ump9622_refout->regsw, &value);
	if (ret) {
		pr_err("Unable to get refout\n");
		return ret;
	}

	mask = (1 << ump9622_refout->refnum) - 1;
...
{
	int ret;
	const int mask_num = 13;
	if ((ump9622_refout == NULL) || (refout_num >= ump9622_refout->refnum)) {
		pr_err("Pmic driver is not init yet or ref num is out\n");
		return -EINVAL;
	}

	if (!refout_state){
		ret = regmap_update_bits(ump9622_refout->regmap, ump9622_refout->regsw,
				 1 << refout_num, 0);
	}
	else if (refout_state == 1)
	{
		...
		//divide the frequence of the refout clk by 2
		regmap_update_bits(ump9622_refout->regmap, ump9622_refout->regdiv,
				 1 << mask_num, 1 << mask_num);
	}
	else {
...
static int sprd_pmic_refout_probe(struct platform_device *pdev)
{
	...
	ump9622_refout = devm_kzalloc(dev, sizeof(struct pmic_refout), GFP_KERNEL);
	if (!ump9622_refout)
		return -ENOMEM;

	ump9622_refout->regmap = dev_get_regmap(dev->parent, NULL);
	if (!ump9622_refout->regmap) {
		pr_err("Get regmap fail\n");
		return -ENODEV;
	}

	ret = of_property_read_u32_index(np, "regsw", 0, &ump9622_refout->regsw);
	if (ret) {
		pr_err("Get base register failed\n");
		return -EINVAL;
	}

	ret = of_property_read_u32_index(np, "regdiv", 0, &ump9622_refout->regdiv);
	if (ret) {
		pr_err("Get TSX_CTRL14 register failed\n");
		return -EINVAL;
	}

	ret = of_property_read_u32_index(np, "refnum", 0, &ump9622_refout->refnum);
	if (ret) {
...
static const struct of_device_id sprd_pmic_refout_match[] = {
	{.compatible = "sprd,ump9622-refout"},
	{},
};
