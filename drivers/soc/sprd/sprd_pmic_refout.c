...
int pmic_refout_update(unsigned int refout_num, int refout_state)
{
    ...
    pr_info("enter pmic refout update\n");
    ...
    if (!refout_state) {
        ret = regmap_update_bits(sc27xx_refout->regmap, sc27xx_refout->regsw,
                 1 << refout_num, 0);
        reg_val = regmap_read(sc27xx_refout->regmap, sc27xx_refout->regsw,
                 &reg_val);
        pr_info("!refout_state and reg_val is: %u\n", reg_val);
    } else if (refout_state == 1) {
        ret = regmap_update_bits(sc27xx_refout->regmap, sc27xx_refout->regsw,
                 1 << refout_num, 1 << refout_num);
        reg_val = regmap_read(sc27xx_refout->regmap, sc27xx_refout->regsw,
                 &reg_val);
        pr_info("refout_state == 1 and reg_val is: %u\n", reg_val);
    } else {
...
static int sprd_pmic_refout_probe(struct platform_device *pdev)
{
    ...
    pr_info("refout driver create group ret value is :%d\n",ret);
    if (ret)
        pr_warn("Failed to create pmic_syscon attributes\n");
    pr_info("pmic probe finish");
    return 0;
}
