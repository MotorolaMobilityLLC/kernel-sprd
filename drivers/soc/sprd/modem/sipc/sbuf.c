...
static void sbuf_comm_init(struct sbuf_mgr *sbuf)
{
	...
	for (i = 0; i < bufnum; i++) {
		...
		ring->tx_wake_lock = wakeup_source_create(ring->tx_wakelock_name);
		wakeup_source_add(ring->tx_wake_lock);

		ring->rx_wake_lock = wakeup_source_create(ring->rx_wakelock_name);
		wakeup_source_add(ring->rx_wake_lock);
	}
}
...
int sbuf_create(u8 dst, u8 channel, u32 bufnum, u32 txbufsize, u32 rxbufsize)
{
	...
	if (!sipc->client) {
		...
		if (ret) {
			for (i = 0; i < bufnum; i++) {
				ring = &sbuf->rings[i];
				wakeup_source_remove(ring->tx_wake_lock);
				wakeup_source_destroy(ring->tx_wake_lock);
				wakeup_source_remove(ring->rx_wake_lock);
...
void sbuf_destroy(u8 dst, u8 channel)
{
	...
	if (sbuf->rings) {
		for (i = 0; i < sbuf->ringnr; i++) {
			...
			wakeup_source_destroy(sbuf->rings[i].tx_wake_lock);
			wakeup_source_remove(sbuf->rings[i].rx_wake_lock);
			wakeup_source_destroy(sbuf->rings[i].rx_wake_lock);
		}
		kfree(sbuf->rings);
	}
