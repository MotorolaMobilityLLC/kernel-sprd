...
static int sprd_send_time_event_msg(struct kobject *kobj)
{
	int ret = 0;
	...
	ret = kobject_uevent_env(kobj, KOBJ_CHANGE, msg);
...
int sprd_send_ap_time(u64 monotime_ns, u64 boottime_ns,
		u64 realtime_s, int tz_minuteswest, int tz_dsttime)
{
	...
	return sprd_send_time_event_msg(&tcd_dev->dev->kobj);
}
		...
		unsigned long action, void *data)
{
	...
	ret = sprd_send_ap_time(monotime_ns, boottime_ns, realtime_s,
