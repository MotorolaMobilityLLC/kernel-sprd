...
static void __dwc3_set_mode(struct work_struct *work)
{
	...
	case DWC3_GCTL_PRTCAP_HOST:
		dwc3_sprd_host_exit(dwc);
		break;
	case DWC3_GCTL_PRTCAP_DEVICE:
		dwc3_sprd_gadget_exit(dwc);
		dwc3_sprd_event_buffers_cleanup(dwc);
		break;
	case DWC3_GCTL_PRTCAP_OTG:
		dwc3_sprd_otg_exit(dwc);
		spin_lock_irqsave(&dwc->lock, flags);
		dwc->desired_otg_role = DWC3_OTG_ROLE_IDLE;
		spin_unlock_irqrestore(&dwc->lock, flags);
		dwc3_sprd_otg_update(dwc, 1);
		break;
	default:
		break;
	}
	...
	dwc3_sprd_set_prtcap(dwc, dwc->desired_dr_role);

	spin_unlock_irqrestore(&dwc->lock, flags);

	switch (dwc->desired_dr_role) {
	case DWC3_GCTL_PRTCAP_HOST:
		ret = dwc3_sprd_host_init(dwc);
		if (ret) {
		...
		break;
	case DWC3_GCTL_PRTCAP_DEVICE:
		dwc3_sprd_event_buffers_setup(dwc);
		...
		ret = dwc3_sprd_gadget_init(dwc);
		if (ret)
			dev_err(dwc->dev, "failed to initialize peripheral\n");
		break;
	case DWC3_GCTL_PRTCAP_OTG:
		dwc3_sprd_otg_init(dwc);
		dwc3_sprd_otg_update(dwc, 0);
		break;
	default:
		break;
...
/**
 * dwc3_sprd_event_buffers_setup - setup our allocated event buffers
 * @dwc: pointer to our controller context structure
 *
...
static int dwc3_setup_scratch_buffers(struct dwc3 *dwc)
{
	...
	ret = dwc3_sprd_send_gadget_generic_command(dwc,
...
static int dwc3_core_init(struct dwc3 *dwc)
{
	...
	ret = dwc3_sprd_event_buffers_setup(dwc);
	if (ret) {
...
static int dwc3_core_init_mode(struct dwc3 *dwc)
{
	...
	case USB_DR_MODE_PERIPHERAL:
		dwc3_sprd_set_prtcap(dwc, DWC3_GCTL_PRTCAP_DEVICE);
	...
	case USB_DR_MODE_HOST:
		dwc3_sprd_set_prtcap(dwc, DWC3_GCTL_PRTCAP_HOST);
...
static void dwc3_check_params(struct dwc3 *dwc)
{
	...
	if (dwc->imod_interval && !dwc3_sprd_has_imod(dwc)) {
...
static int dwc3_resume_common(struct dwc3 *dwc, pm_message_t msg)
{
	...
	case DWC3_GCTL_PRTCAP_OTG:
		...
		dwc3_sprd_set_prtcap(dwc, dwc->current_dr_role);

		dwc3_sprd_otg_init(dwc);
		if (dwc->current_otg_role == DWC3_OTG_ROLE_HOST) {
			dwc3_sprd_otg_host_init(dwc);
...
static int dwc3_runtime_resume(struct device *dev)
{
	...
	switch (dwc->current_dr_role) {
	case DWC3_GCTL_PRTCAP_DEVICE:
		dwc3_sprd_gadget_process_pending_events(dwc);
		break;
	case DWC3_GCTL_PRTCAP_HOST:
	default:
		/* do nothing */
...
MODULE_ALIAS("platform:dwc3-unisoc");
