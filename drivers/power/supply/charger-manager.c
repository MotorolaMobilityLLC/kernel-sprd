...
static int set_batt_total_cap(struct charger_manager *cm, int total_cap)
{
	...
	ret = power_supply_set_property(fuel_gauge, POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN, &val);
	power_supply_put(fuel_gauge);
	if (ret)
...
static void cm_batt_works(struct work_struct *work)
{
	...
	dev_info(cm->dev, "vbat=%d,OCV=%d,ibat=%d,ibus=%d,vbus=%d,"
		 ...
		 batt_uV/1000, batt_ocV/1000, bat_uA/1000, input_cur/1000,  chg_vol/1000, fuel_cap,cm->desc->cap ,cm->desc->charger_status,
