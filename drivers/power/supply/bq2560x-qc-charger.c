...
static int bq2560xqc_set_qc(struct bq2560xqc_charger_info *info,int voltage)
{
	...
	ret = power_supply_set_property(info->psy_bq2560x, POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE,
					&val);
	return ret;
...
static int bq2560xqc_set_qc_continue(struct bq2560xqc_charger_info *info,int step,int direction)
{
	...
	val.intval = (step << 8) | direction;

	ret = power_supply_set_property(info->psy_bq2560x, POWER_SUPPLY_PROP_CHARGE_COUNTER,
					&val);
	return ret;
}
