			   ...
			   struct list_head *msg_buf_list)
{
	...
	mutex_lock(&vds->lock);
	if (vds->state == VDS_ONLINE) {
		sg = kzalloc(sizeof(*sg)*vds->msg_buf_max_cnt, GFP_KERNEL);
		if (!sg) {
			err = -ENOMEM;
			mutex_unlock(&vds->lock);
			goto err_out;
...
int tipc_chan_queue_msg(struct tipc_chan *chan, struct list_head *msg_buf_list)
{
	int err;
	struct tipc_msg_buf *mb;
...
int tipc_chan_connect(struct tipc_chan *chan, const char *name)
{
	...
	case TIPC_DISCONNECTED:
		...
		pr_info("%s: queue tx buffer (0x%lx)\n",
...
static ssize_t tipc_write_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	/* queue message */
	ret = tipc_chan_queue_msg(dn->chan, msg_buf_list);
	if (ret)
		goto err_out;
...
static void _rxvq_cb(struct virtqueue *rxvq)
{
	...
	while ((msg_buf_list = virtqueue_get_buf(rxvq, &len)) != NULL) {
		...
		sg = kzalloc(sizeof(*sg) * rxvq_num, GFP_KERNEL);
		if (!sg) {
			err = -ENOMEM;
			goto err_sg;
		}
