...
#ifdef CONFIG_SEC_NFC_GPIO_CLK
	pr_err("sec_nfc clk start!");
    pdata->clk_req = of_get_named_gpio(np, "sec-nfc,clk_req-gpio", 0);
    pdata->clk = clk_get(dev, "nfc_ref_clk");
	pr_err("sec_nfc clk end!");
#endif
                            ...
                            const struct i2c_device_id *id)
{
	...
	//add nfc_clok
	/*
	pr_err(" sec_nfc before open refout4 1026_4");
	pmic_refout_update(4, 1);
	pr_err("sec_nfc after open refout4 1026_4");
	*/
