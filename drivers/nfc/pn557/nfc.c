    ...
    struct nfc_platform_data *data)
{
    ...
    data->clkreq_gpio = of_get_named_gpio(np, "nxp,pn557-clkreq", 0);
        ...
        const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(platform_data.clkreq_gpio)) {
        ...
        if (ret) {
            pr_err("%s: cannot set direction for nfc clkreq gpio [%d]\n",
            __func__, platform_data.clkreq_gpio);
            goto err_clkreq_gpio;
        }
    } else {
        pr_err("%s: clkreq gpio not provided\n", __func__);
        goto err_firm_gpio;
    }
