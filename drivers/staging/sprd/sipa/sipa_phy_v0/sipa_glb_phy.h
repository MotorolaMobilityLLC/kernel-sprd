 ...
 *   FALSE: Set failed.
 */
static inline u32 ipa_phy_set_work_mode(
	...
	u8 is_bypass)
{
	...
	switch (is_bypass) {
	case normal_mode:
		ret = readl_relaxed(reg_base +
							IPA_MODE_N_FLOWCTRL);
		ret &= (~IPA_WORKING_MODE_MASK);
		writel_relaxed(ret,
					   reg_base + IPA_MODE_N_FLOWCTRL);
		ret = readl_relaxed(reg_base +
							IPA_MODE_N_FLOWCTRL);
 ...
 */
static inline u32 ipa_phy_get_int_status(
	void __iomem *reg_base)
{
	u32 ret;
 ...
 *   @FALSE: pause ipa.
 */
static inline u32 ipa_phy_ctrl_ipa_action(
	void __iomem *reg_base,
	u32 enable)
{
	u32 ret;
...
static inline u32 ipa_phy_enable_cp_through_pcie(
	void __iomem *reg_base,
	u32 enable)
{
	u32 ret;
 ...
 */
static inline u32 ipa_phy_get_hw_ready_to_check_sts(
	void __iomem *reg_base)
{
	u32 tmp = 0;

	tmp = readl_relaxed(reg_base +
						IPA_MODE_N_FLOWCTRL);
 ...
 */
static inline u32 ipa_phy_hash_table_switch(
	void __iomem *reg_base,
	...
	u32 len)
{
	...
	writel_relaxed(addr_l,
				   reg_base + IPA_HASH_TABLE_BASE_ADDR_LOW);
	...
	writel_relaxed(tmp,
				   reg_base + IPA_HASH_TABLE_SWITCH_CTL);
	...
	writel_relaxed(tmp, reg_base +
				   IPA_HASH_TABLE_SWITCH_CTL);

	tmp = readl_relaxed(reg_base +
						IPA_HASH_TABLE_SWITCH_CTL);

	while ((tmp & IPA_HASH_REG_DONE_MASK) ==
		   ZERO) {
		tmp = readl_relaxed(reg_base +
							IPA_HASH_TABLE_SWITCH_CTL);
	}
 ...
 *   @len: Hash Table length
 */
static inline u32 ipa_phy_get_hash_table(
	void __iomem *reg_base,
	u32 *addr_l, u32 *addr_h,
	u32 *len)
{
 ...
 */
static inline u32 ipa_phy_map_interrupt_src_en(
	void __iomem *reg_base,
	u32 enable, u32 mask)
{
	u32 tmp = 0;
	...
	if (enable) {
		...
		tmp |= mask;
		writel_relaxed(tmp,
					   reg_base + IPA_MAP_INTERRUPT_SRC_EN);
		tmp = readl_relaxed(reg_base +
							IPA_MAP_INTERRUPT_SRC_EN);
		if (tmp & mask)
			return TRUE;
 ...
 */
static inline u32 ipa_phy_clear_internal_fifo(
	void __iomem *reg_base,
	u32 clr_bit)
{
	u32 tmp;
	...
	clr_bit &=
		IPA_INTERNAL_FIFO_CLR_BIT_GROUP_MASK;
	tmp = readl_relaxed(reg_base +
						IPA_INTERNAL_FIFO_CLR);
	tmp |= clr_bit;
	writel_relaxed(tmp,
				   reg_base + IPA_INTERNAL_FIFO_CLR);
	tmp &= (~clr_bit);
	writel_relaxed(tmp,
				   reg_base + IPA_INTERNAL_FIFO_CLR);
...
static inline u32 ipa_phy_set_force_to_ap_flag(
	void __iomem *reg_base,
	u32 enable, u32 bit)
{
	u32 tmp;
 ...
 *   @src: send fifo.
 */
static inline u32 ipa_phy_set_flow_ctrl_to_src_blk(
	void __iomem *reg_base,
	u32 *dst, u32 src)
{
	u32 tmp;
 ...
 */
static inline u32 ipa_phy_get_flow_ctrl_to_src_sts(
	void __iomem *reg_base,
	u32 *dst, u32 src)
{
	u32 tmp = 0;
 ...
 *   @prio: Priority value.
 */
static inline u32 ipa_phy_set_axi_mst_chn_priority(
	void __iomem *reg_base,
	u32 chan, u32 prio)
{
	/*not ready*/
	...
	writel_relaxed(tmp,
				   reg_base + IPA_AXI_MST_CHN_PRIORITY);
 ...
 */
static inline u32 ipa_phy_ctrl_wiap_ul_dma(
	void __iomem *reg_base,
	u32 enable)
{
	u32 flag, tmp;
...
static inline u32 ipa_phy_get_timestamp(
	void __iomem *reg_base)
{
