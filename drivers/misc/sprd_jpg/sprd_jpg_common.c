...
int jpg_get_mm_clk(struct jpg_dev_t *jpg_hw_dev)
{
	...
	if (IS_ERR_OR_NULL(jpg_domain_eb)) {
...
int jpg_clk_enable(struct jpg_dev_t *jpg_hw_dev)
{
	...
	if (ret) {
		pr_err("jpg jpg_domain_eb clk_prepare_enable");
		pr_err(" failed!\n");
		return ret;
	}
