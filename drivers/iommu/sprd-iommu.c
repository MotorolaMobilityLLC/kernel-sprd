...
static inline void *fwspec_to_sdev(struct iommu_fwspec *fwspec)
{
				    ...
				    struct device *dev)
{
	...
	struct sprd_iommu_device *sdev = fwspec_to_sdev(fwspec);
	struct sprd_iommu_domain *dom = to_sprd_domain(domain);
...
static int sprd_iommu_add_device(struct device *dev)
{
	struct iommu_fwspec *fwspec = dev_iommu_fwspec_get(dev);
	struct sprd_iommu_device *sdev;
	struct iommu_group *group;
	 ...
	 * needs.
	 */
	link = device_link_add(dev, sdev->dev, DL_FLAG_PM_RUNTIME);
	if (!link) {
		dev_err(sdev->dev, "Unable to create device link between %s and %s\n",
			dev_name(sdev->dev), dev_name(dev));
		return -ENODEV;
	}
	...
	iommu_device_link(&sdev->iommu, dev);

	return 0;
}

static void sprd_iommu_remove_device(struct device *dev)
{
	struct iommu_fwspec *fwspec = dev_iommu_fwspec_get(dev);
	struct sprd_iommu_device *sdev;
	...
	iommu_device_unlink(&sdev->iommu, dev);
...
static const struct iommu_ops sprd_iommu_ops = {
	...
	.add_device	= sprd_iommu_add_device,
	.remove_device	= sprd_iommu_remove_device,
