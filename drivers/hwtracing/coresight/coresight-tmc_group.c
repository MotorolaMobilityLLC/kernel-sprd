...
#if 0
/* check if cp etb is ON or OFF in ylog */
static bool tmc_global_check_ctrl_enable(struct tmc_drvdata_group *drvdata)
{
	int i;
	u32 reg_val = 0;
	u32 reg_val2 = 0;
	...
	for (i = 0; i < drvdata->cs_element_num; i++) {
		name_ptr = drvdata->cs_context[i].config.name;
		if (!strcmp("soc_etb_regs", name_ptr)) {
			reg_val = readl_relaxed(drvdata->cs_context[i].config.vir_start + TMC_CTL);
			if (reg_val & TMC_CTL_CAPT_EN)
				return true;
		}

		if (!strcmp("soc_funnel", name_ptr)) {
			reg_val2 = readl_relaxed(drvdata->cs_context[i].config.vir_start);
			if (reg_val2 & BIT(7))
				return true;
		}
	}
...
#if 0
	/* if tmc_ctrl_enable false, stoping dumping etb log */
	if (tmc_global_check_ctrl_enable(drvdata)) {
		pr_info("tmc_ctrl_enable ok\n");
	} else {
		pr_err("tmc_ctrl_enable fail\n");
		return -ENODEV;
	}
#endif
...
static int tmc_open(struct inode *inode, struct file *file)
{
	...
	if (ret) {
		dev_info(drvdata->dev, "tmc_read_prepare fail!\n");
		return ret;
	}
