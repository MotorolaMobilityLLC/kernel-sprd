					    ...
					    unsigned int timeout_us)
{
	...
	for (;;) {
		...
		if (timeout_us && ktime_compare(ktime_get(), timeout) > 0) {
			...
			if (val >= 0)
				cond = ((val & check1) && (val & check2));
			sp_asoc_pr_dbg("%s val %d 0x%x, check1 0x%x, check2 0x%x, cond 0x%x\n",
...
static void sprd_dalr_dc_os_set(struct snd_soc_codec *codec)
{
	...
	if (NEED_SDM_RAMP(sprd_codec, codec)) {
		/* kcontrol set "DALDO BYPASS" 1: DALDO_BYPASS, 0: DALDO_EN */
			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s %s\n", w->name, get_event_name(event));
	if (on) {
		sprd_codec_sdm_init(codec);/* check further */
...
static int sdahp_dpop_event(struct snd_soc_dapm_widget *w,
			    struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	int on = !!SND_SOC_DAPM_EVENT_ON(event);
	...
	sp_asoc_pr_info("wname %s %s\n", w->name, get_event_name(event));
	/* check further, separate L R in two widget? */
	if (on)
		update_switch(codec, SDALHPL_HPL | SDARHPR_HPR, on);
	else
		update_switch(codec, SDALHPL_HPL | SDARHPR_HPR, on);
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s event %s, dahp_os_inv %d, dahp_os_d %d\n",
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/*
		 * controls: "DAHP OS D" need set to "7" in "de-pop"
		 * set to 0 in "only using HP path", rcv is 7
		 */
		snd_soc_update_bits(codec, SOC_REG(ANA_DAC1), DAHP_OS_D(0x7),
				    DAHP_OS_D(sprd_codec->dahp_os_d));
		break;
	case SND_SOC_DAPM_PRE_PMD:
			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s event %s, drv_soft_t %d\n", w->name,
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s event %s, daaol_buf_dccal %s\n", w->name,
			get_event_name(event),
			da_buf_cal_mode_txt[sprd_codec->daaol_buf_dccal]);
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		switch (sprd_codec->daaol_buf_dccal) {
		case HW_CAL_MODE:
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s event %s, daaor_buf_dccal %s\n",
			w->name, get_event_name(event),
			da_buf_cal_mode_txt[sprd_codec->daaor_buf_dccal]);

	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		switch (sprd_codec->daaor_buf_dccal) {
		case HW_CAL_MODE:
			snd_soc_update_bits(codec, SOC_REG(ANA_CDC5),
			...
			snd_soc_update_bits(codec, SOC_REG(ANA_CDC5),
					    ...
					    DAAOR_BUF_DCCAL_EN);
			/* check further */
			ret = regmap_read_poll_timeout(codec_regmap,
						       SOC_REG(ANA_CDC7), val,
					((val & DAAOR_VRPBUF_CAL_DONE) &&
					(val & DAAOR_VRNBUF_CAL_DONE)),
					DAC_CAL_POLL_DELAY_US,
					DAC_CAL_DONE_TIMEOUT);
			if (ret)
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s event %s, dahpl_buf_dccal %s\n", w->name,
			get_event_name(event),
			da_buf_cal_mode_txt[sprd_codec->dahpl_buf_dccal]);

	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		switch (sprd_codec->dahpl_buf_dccal) {
		case HW_CAL_MODE:
			snd_soc_update_bits(codec, SOC_REG(ANA_CDC5),
			...
			ret = regmap_read_poll_timeout(codec_regmap,
						       SOC_REG(ANA_CDC7), val,
					((val & DAHPL_VRPBUF_CAL_DONE) &&
					(val & DAHPL_VRNBUF_CAL_DONE)),
					DAC_CAL_POLL_DELAY_US,
					DAC_CAL_DONE_TIMEOUT);
			if (ret)
				pr_err("%s check cal done fail, val 0x%x, %d\n",
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s event %s, dahpr_buf_dccal %s\n",
			w->name, get_event_name(event),
			da_buf_cal_mode_txt[sprd_codec->dahpr_buf_dccal]);

	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		switch (sprd_codec->dahpr_buf_dccal) {
		case HW_CAL_MODE:
			snd_soc_update_bits(codec, SOC_REG(ANA_CDC5),
			...
			ret = regmap_read_poll_timeout(codec_regmap,
						       SOC_REG(ANA_CDC7), val,
					((val & DAHPR_VRPBUF_CAL_DONE) &&
					 (val & DAHPR_VRNBUF_CAL_DONE)),
					DAC_CAL_POLL_DELAY_US,
					DAC_CAL_DONE_TIMEOUT);
			if (ret)
				pr_err("%s check cal done fail, val 0x%x, %d\n",
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s event %s, clk_26m_in_sel %d\n", w->name,
		      ...
		      struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s sname %s, %s %s\n", w->name, w->sname,
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	sp_asoc_pr_info("wname %s %s, check %s i %d\n", w->name,
			get_event_name(event),
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* check further */
		ret = regmap_read_poll_timeout(codec_regmap, SOC_REG(ANA_STS1),
					       val,
		       ((val & RCV_DCCAL_DVLD) && (val & RCV_LOOP_DVLD)),
...
/* DCL clock */
	...
	/* check further, wsubseq index */
	SND_SOC_DAPM_SUPPLY_S("DRV SOFT EN", 5,
...
static int sprd_codec_probe(struct platform_device *pdev)
{
	...
	ana_chip_id  = sci_get_ana_chip_id();
	pr_info("ana_chip_id 0x%x %x\n", ana_chip_id >> 16,
		ana_chip_id & 0xffff);
