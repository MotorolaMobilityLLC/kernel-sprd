...
static const char *const aw881xx_i2s_rate[] = {"I2S_8K", "I2S_16K", "I2S_32K", "I2S_44K1", "I2S_48K",
						"I2S_96K", "I2S_192K"};
static const char *const aw881xx_i2s_width[] = {"I2S_16Bit", "I2S_20Bit", "I2S_24Bit", "I2S_32Bit"};
...
static int aw881xx_read_i2s_rate(struct aw881xx *aw881xx, uint16_t *rate)
{
	uint16_t reg_val = 0;
	int ret;

	ret = aw881xx_reg_read(aw881xx, AW881XX_REG_I2SCTRL, &reg_val);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: get i2s rate failed\n",
				__func__);
		return ret;
	}

	*rate = reg_val & (~AW881XX_BIT_I2SCTRL_SR_MASK);

	return 0;
}

static int aw881xx_read_cco_mute_rate(struct aw881xx *aw881xx,
						uint16_t *cco_rate)
{
	int ret;
	uint16_t reg_val = 0;

	ret = aw881xx_reg_read(aw881xx, AW881XX_REG_PLLCTRL1, &reg_val);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: get cco mute rate failed\n",
				__func__);
		return ret;
	}

	*cco_rate = reg_val & (~AW881XX_I2S_CCO_MUX_MASK);

	return 0;
}

static int aw881xx_read_i2s_width(struct aw881xx *aw881xx, uint16_t *width)
{
	int ret;
	uint16_t reg_val = 0;
	...
	ret = aw881xx_reg_read(aw881xx, AW881XX_REG_I2SCTRL, &reg_val);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: read i2s width failed\n",
				__func__);
		return ret;
	}

	*width = reg_val & (~AW881XX_BIT_I2SCTRL_FMS_MASK);
...
static void aw881xx_stop(struct aw881xx *aw881xx)
{
	...
	aw881xx_dsp_cfg_update_when_stop(aw881xx);
...
static int aw881xx_i2s_rate_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
	int count;
	...
	uinfo->type = SNDRV_CTL_ELEM_TYPE_ENUMERATED;
	uinfo->count = 1;
	count = ARRAY_SIZE(aw881xx_i2s_rate);
	...
	strlcpy(uinfo->value.enumerated.name,
		aw881xx_i2s_rate[uinfo->value.enumerated.item],
		sizeof(uinfo->value.enumerated.name));

	return 0;
}

static int aw881xx_i2s_rate_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	struct aw881xx *aw881xx =
		aw_componet_codec_ops.aw_snd_soc_codec_get_drvdata(codec);
	int ret;
	uint16_t real_i2s_rate = 0;
	uint16_t rate_id = 0;

	ret = aw881xx_read_i2s_rate(aw881xx, &real_i2s_rate);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: get i2s rate failed\n",
				__func__);
		return ret;
	...
	switch (real_i2s_rate) {
	case AW881XX_BIT_I2SCTRL_SR_8K:
		rate_id = I2S_RATE_8K;
		break;
	case AW881XX_BIT_I2SCTRL_SR_16K:
		rate_id = I2S_RATE_16K;
		break;
	case AW881XX_BIT_I2SCTRL_SR_32K:
		rate_id = I2S_RATE_32K;
		break;
	case AW881XX_BIT_I2SCTRL_SR_44P1K:
		rate_id = I2S_RATE_44K1;
		break;
	case AW881XX_BIT_I2SCTRL_SR_48K:
		rate_id = I2S_RATE_48K;
		break;
	case AW881XX_BIT_I2SCTRL_SR_96K:
		rate_id = I2S_RATE_96K;
		break;
	case AW881XX_BIT_I2SCTRL_SR_192K:
		rate_id = I2S_RATE_192K;
		break;
	default:
	...
	ucontrol->value.integer.value[0] = rate_id;
	aw_dev_dbg(codec->dev, "%s:get i2s rate[%d]:%s\n",
			__func__, rate_id, aw881xx_i2s_rate[rate_id]);

	return 0;

}

static int aw881xx_i2s_rate_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	...
	uint16_t reg_val = 0;
	uint16_t real_i2s_rate = 0;
	uint16_t real_cco_value = 0;
	uint16_t cco_mux_value = 0;
	int ret;

	aw_dev_dbg(aw881xx->dev, "%s: value:%ld\n",
			__func__, ucontrol->value.integer.value[0]);
	...
	ret = aw881xx_read_cco_mute_rate(aw881xx, &real_cco_value);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: get cco mute value failed\n",
				__func__);
		mutex_unlock(&aw881xx->lock);
		return ret;
	}

	if ((reg_val == real_i2s_rate) &&
		(cco_mux_value == real_cco_value)) {
		aw_dev_info(aw881xx->dev, "%s: i2s_rate and cco_mute_value no change\n",
				__func__);
	} else {
...
static int aw881xx_i2s_width_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
	int count;
	...
	uinfo->type = SNDRV_CTL_ELEM_TYPE_ENUMERATED;
	uinfo->count = 1;
	count = ARRAY_SIZE(aw881xx_i2s_width);
	...
	strlcpy(uinfo->value.enumerated.name,
		aw881xx_i2s_width[uinfo->value.enumerated.item],
		sizeof(uinfo->value.enumerated.name));

	return 0;
}

static int aw881xx_i2s_width_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	struct aw881xx *aw881xx =
		aw_componet_codec_ops.aw_snd_soc_codec_get_drvdata(codec);
	int ret;
	uint16_t real_i2s_width = 0;
	uint16_t width_id = 0;

	ret = aw881xx_read_i2s_width(aw881xx, &real_i2s_width);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev, "%s: get i2s width failed\n",
				__func__);
		return ret;
	}

	switch (real_i2s_width) {
	case AW881XX_BIT_I2SCTRL_FMS_16BIT:
		width_id = I2S_WIDTH_16BIT;
		break;
	case AW881XX_BIT_I2SCTRL_FMS_20BIT:
		width_id = I2S_WIDTH_20BIT;
		break;
	case AW881XX_BIT_I2SCTRL_FMS_24BIT:
		width_id = I2S_WIDTH_24BIT;
		break;
	case AW881XX_BIT_I2SCTRL_FMS_32BIT:
		width_id = I2S_WIDTH_32BIT;
		break;
	default:
		aw_dev_err(aw881xx->dev, "%s: rate can not support\n",
				__func__);
	...
	ucontrol->value.integer.value[0] = width_id;
	aw_dev_dbg(codec->dev, "%s:get i2s width:%s\n",
			__func__, aw881xx_i2s_width[width_id]);

	return 0;

}

static int aw881xx_i2s_width_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
	...
	if (real_i2s_width == reg_val) {
		aw_dev_info(aw881xx->dev, "%s: i2s_width no change\n",
				__func__);
	} else {
		/* set width */
		aw881xx_reg_write_bits(aw881xx, AW881XX_REG_I2SCTRL,
...
static int aw881xx_dynamic_create_common_controls(struct aw881xx *aw881xx)
{
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_i2s_rate", aw881xx->channel);

	aw881xx_dev_control[2].name = kctl_name;
	aw881xx_dev_control[2].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw881xx_dev_control[2].info = aw881xx_i2s_rate_info;
	aw881xx_dev_control[2].get = aw881xx_i2s_rate_get;
	aw881xx_dev_control[2].put = aw881xx_i2s_rate_set;
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_i2s_width", aw881xx->channel);

	aw881xx_dev_control[3].name = kctl_name;
	aw881xx_dev_control[3].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw881xx_dev_control[3].info = aw881xx_i2s_width_info;
	aw881xx_dev_control[3].get = aw881xx_i2s_width_get;
	aw881xx_dev_control[3].put = aw881xx_i2s_width_set;
