...
{
	make_config
	make -C $BSP_KERNEL_PATH O=$BSP_KERNEL_OUT $BSP_MAKE_EXTRA_ARGS ARCH=$BSP_KERNEL_ARCH CROSS_COMPILE=$BSP_KERNEL_CROSS_COMPILE -j$BSP_OBJ
	compile_check $?
...
{
	make -C $BSP_KERNEL_PATH O=$BSP_KERNEL_OUT $BSP_MAKE_EXTRA_ARGS ARCH=$BSP_KERNEL_ARCH CROSS_COMPILE=$BSP_KERNEL_CROSS_COMPILE $BSP_KERNEL_DEFCONFIG -j$BSP_OBJ
	compile_check $?
...
{
	make -C $BSP_KERNEL_PATH O=$BSP_KERNEL_OUT $BSP_MAKE_EXTRA_ARGS ARCH=$BSP_KERNEL_ARCH CROSS_COMPILE=$BSP_KERNEL_CROSS_COMPILE $BSP_KERNEL_DEFCONFIG menuconfig -j$BSP_OBJ
	compile_check $?
...
function make_dtb()
{
	make -C $BSP_KERNEL_PATH O=$BSP_KERNEL_OUT $BSP_MAKE_EXTRA_ARGS ARCH=$BSP_KERNEL_ARCH CROSS_COMPILE=$BSP_KERNEL_CROSS_COMPILE dtbs -j$BSP_OBJ
	compile_check $?
...
function make_modules()
{
	if [ -n $BSP_KERNEL_DIST ]; then
		mkdir $BSP_KERNEL_DIST -p
		make -C $BSP_KERNEL_OUT O=$BSP_KERNEL_OUT $BSP_MAKE_EXTRA_ARGS INSTALL_MOD_PATH=$BSP_KERNEL_DIST modules_install -j$BSP_OBJ
		compile_check $?
	fi
}
...
function make_clean()
{
	make -C $BSP_KERNEL_PATH O=$BSP_KERNEL_OUT $BSP_MAKE_EXTRA_ARGS ARCH=$BSP_KERNEL_ARCH CROSS_COMPILE=$BSP_KERNEL_CROSS_COMPILE mrproper -j$BSP_OBJ
	compile_check $?
}
